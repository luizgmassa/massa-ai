...
describe("roster: nothing advertises a specialist count other than 18", () => {
   ...
   *
   * The strictly line-scoped successor was ALSO blind: `CLAUDE.md` wrapped
   * "... those same 17\nspecialists plus hooks per host, generated by" across
   * a hard line break, so no single line held both the digit and the noun,
   * and this gate stayed green at 1810/0 with the defect live
   * (installer-prune-and-test-scoping IPT-06). Widening `\s+` to an unbounded
   * `\s+` (crossing blank lines, i.e. paragraph boundaries) was rejected in
   * the very docblock this replaces, because it pairs an unrelated
   * `command_count=0` with the word "specialist" forty lines later — that
   * risk is real and is exactly what AC-06.5/AC-06.6 guard against.
   *
   * `WS` below is the bounded middle ground: it matches ordinary same-line
   * whitespace, OR exactly one newline optionally followed by leading
   * indentation — never two consecutive newlines (a blank line), so a
   * paragraph break still stops the match. The measured target
   * (`CLAUDE.md:331`-`:332`) spans exactly one such newline.
   */
  const WS = "(?:[ \\t]+|\\n[ \\t]*)";
  const COUNT_CLAIM = new RegExp(
    `(\\d+)${WS}(?:reusable${WS})?(?:sub-?agent${WS})?[Ss]pecialists?\\b`
  );
  ...
  test(`no tracked source, doc, or installer claims a roster other than ${ROSTER}`, async () => {
    ...
    for (const rel of files) {
      ...
      // Scan the whole file, not line-by-line: COUNT_CLAIM may now span one
      // newline (AC-06.1), so a per-line split would re-introduce the blind
      // spot this gate exists to close.
      const scan = new RegExp(COUNT_CLAIM.source, "g");
      ...
      while ((m = scan.exec(body))) {
        ...
        // Union of every line the match touches (one line for an ordinary
        // same-line claim, two for a claim split across the bounded newline)
        // — HISTORICAL substrings are checked against that same span so a
        // wrapped historical claim is exempted exactly like an unwrapped one.
        const context = body.slice(lineStart, lineEnd);
        if (HISTORICAL.some((h) => context.includes(h))) continue;
        if (m[1] !== String(ROSTER)) {
          const lineNo = body.slice(0, m.index).split("\n").length;
          // Report starting at the claim itself (not the start of its line),
          // so a long lead-in sentence can never push the digit and noun past
          // the 100-char slice — the whole point of surfacing this offender.
          const claim = body.slice(m.index, lineEnd).replace(/\s+/g, " ").trim().slice(0, 100);
          offenders.push(`${rel}:${lineNo}: ${claim}`);
        }
      }
    }
