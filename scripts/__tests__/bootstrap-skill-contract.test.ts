 ...
 *
 * 2. **The documented id list is the registry's six (BST-12 AC-4).** Parsed
...
describe("skills/bootstrap/SKILL.md — documented ids match the registry (BST-12 AC-4)", () => {
  test("the documented id list is exactly the registry's six, in registry order", () => {
    ...
    expect(documented.length).toBe(6);
  });
