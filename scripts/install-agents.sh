...
# ── Advisory: where the 15 subagent specialists come from ──────────────────
...
specialist_hint() {
  ...
  [ "$VERBOSE" != "1" ] && return 0
  case "$1" in
    claude-code) echo "  💡 15 subagent specialists: apps/claude-plugin/install.sh --user" ;;
    codex) echo "  💡 15 subagent specialists: apps/codex-plugin/install.sh --user" ;;
    cursor) echo "  💡 15 subagent specialists: apps/cursor-plugin/install.sh --user" ;;
    opencode) echo "  💡 15 subagent specialists: massa-ai-config agents install --user" ;;
    *) echo "" ;;
  esac
}
