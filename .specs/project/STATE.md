...
## Previous — Agent roster revision (**COMPLETE 2026-09-23** — 11 Tasks in 4 parallel waves, independent verification PASS)
...
- projectId: `massa-ai` · workflowSessionId: `spec-workflow-commands` · workflow:
  ...
  (v1.29.0), worktree `.claude/worktrees/workflow-commands`. Post-PR: main
  v1.32.0 (AD-017 PR #74, pr-review PR #75, agent-era PR #76, discovery PR #77)
  merged into the branch — installer conflicts resolved onto the AD-017
  structure (route-dependent messages keep `${command_count}`; T9 `profile`
  exclusion re-grafted into the bridge-era local-route loop); workflow
  inventory 38 → 40, pr-review + discovery commands generated by the scan
  with zero generator changes (the AD-018 property working as designed).
...
## Previous — Agent-Era Harness Upgrades (**VALIDATED PASS 2026-08-06** — 28/28 ACs, 8/8 mutants killed; merged, released v1.31.0–v1.32.0; file-size thresholds amended ~500/~600 by user @ `eb23dd0a`)
...
| AD-020 | active (admin-portal-ops-suite T21, implemented 2026-08-10, branch `spec/admin-portal-ops-suite`) | **A Claude marketplace-route install is switchable; the switch target is the host's own versioned plugin cache, resolved from `installed_plugins.json` at every call and never cached.** This narrows AD-015's `installRoute` semantics: `"marketplace"` is no longer a synonym for "unswitchable". The original refusal's stated reason — an in-place bundle rewrite would dirty the checkout and break the drift gate — was **measured false for Claude**: `installLocation` points at the checkout, but `installPath` is a real copy under `~/.claude/plugins/cache/<marketplace>/<plugin>/<version>/` carrying both `agents/` (17 `massa-ai-*.md`) and `agent-profiles/` (5 variants). `hosts.ts` stays filesystem-free — callers pass a pre-resolved `marketplaceRoot`, and `detectRoute` gains an **optional trailing** `host?` parameter (leading would have put a `PlatformRecord` in the host slot for any out-of-tree consumer of the published `@massa-ai/shared` re-export, making every host refuse). An unresolvable root reports `installed:false` / `failed` naming the unresolved path — never the file-route fallback. Codex's marketplace route **keeps** the refusal, with a reason naming codex only, because the same premise is unverified there. Rejected alternative: converting Claude to the file route, which would leave the bundle's agents and loose `~/.claude/agents/` copies both discoverable — 17 duplicate specialists. Consequence: `claude plugin update` overwrites the cache, so the Claude installer re-applies the recorded `platforms.claude.modelProfile` after every update (AD-015's installer-reads-never-writes rule is preserved). **Unverifiable from this repository:** Claude's cache-refresh cadence — the switch carries a mandatory post-restart acceptance step (see HANDOFF). | `.specs/features/admin-portal-ops-suite/{spec,design,validation}.md`; `packages/shared/src/profile-switch/{claude-marketplace,hosts,engine,variant-sync}.ts`; `apps/claude-plugin/install.sh`; discrimination sensor 7/7 mutations killed |
| AD-021 | active (web-ui-typescript T6/T35/T36, implemented 2026-08-12, branch `spec/web-ui-typescript`) | **AD-016's "consumers chain the generation" contract is not specific to `apps/*-plugin` — it applies to every untracked generated root, and `apps/web-ui/dist/` is the second instance.** `dist/static` is gitignored `tsc` output (`apps/web-ui/tsconfig.build.json` emitting `src/static/**/*.ts`); every consumer that reads it — the served app (`apps/tools-api/src/routes/web-ui.ts`), the module-graph guard, `check-coverage.ts` — must chain a build ahead of itself or read stale/absent output. AD-016's own enforcement pattern (a pre-script that runs the generator before its consumer) does not transfer directly: its consumers are npm/turbo scripts that can prepend a shell command, but `static-module-graph.test.ts` is invoked directly by `bun test` with no script to inject into. The pattern this feature used instead is a **loud `beforeAll` sentinel**: it checks for `dist/static` and throws, naming the exact build command (`bun run --filter @massa-ai/web-ui build`), rather than silently scanning zero files or leaving the reader to a bare `ENOENT`. `CHARACTERIZATION.md`'s own verification recipe needed the same correction (T35) — its baseline commands are bare `bun test`, not turbo-mediated, and fail on a clean unbuilt tree without a human reading the sentinel's message first. Re-checked at append time: AD-020 was the highest existing decision (20 distinct `AD-` ids in this file), confirmed by count, not assumption — AD-014 records a pre-assigned number already being taken once before. **Addendum (2026-08-12, Phases 15-17, T45):** after this decision was recorded, the operator found three live-server defects (Model Catalog override count, Config tab defaults, Logs Live preference) that reproduce byte-for-byte on `origin/main`; fixing them (T41-T44) by explicit user decision retired the branch's behaviour-preserving property for exactly 3 of 86 `render-golden.json` entries, named in advance by the fixing task — see `.specs/features/web-ui-typescript/{tasks,validation}.md` (Execution Log Batch 15, `validation.md` claim 2) and `spec.md`'s WUT-17..19 traceability rows (now Verified). This addendum does not change the AD-016-generalization decision itself. | `apps/web-ui/src/__tests__/static-module-graph.test.ts` (`beforeAll` sentinel, T6); `.specs/features/web-ui-typescript/{design,tasks,CHARACTERIZATION}.md` |

<!-- Placement note (2026-08-10): AD-020 is appended here, after AD-019, because
     STATE.md has TWO `## Decisions` headings — L3735 holds AD-007..AD-015, and
     this one (L3845) holds AD-001..AD-006 plus AD-016..AD-019. No single table
     holds "AD-007..AD-019". Keeping AD-020 adjacent to its immediate
     predecessors is what makes it findable; the duplicate heading is a
     pre-existing rotation defect recorded in the design's Risks table and in
     HANDOFF, not something this feature repairs. -->
