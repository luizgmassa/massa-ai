...
**Done when**:
- [ ] The ladder is keyed on `command -v bun`, never on `installer_detect_runner` — that helper returns `node` first (`scripts/lib/installer-shared.sh:25-33`) and node is always present here as the node-gyp helper, so a `$RUNNER`-keyed ladder would take the bun branch on no machine
