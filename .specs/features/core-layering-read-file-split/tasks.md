...
| **6 — the rename** | ~~29~~ → **30, COMPLETE at T18** | 0 | **7** `git mv` + ~~19~~ → **20** external editors (**+ `docs/ONBOARDING.md:225`**, the unowned live prose site — **C82**, §10.19) + **3** prose/fixture sites (**T18 DONE**, §10.24) |
| **7 — the record** | ~~**16, unchanged by T19**~~ → **18 at T21** | 1 | RFS-04's 5 + RFS-05's 9 + `design.md` (T20b) + `validation.md`. **T19 shipped exactly its 5** (§10.20) — the first Phase-7 landing, zero growth. **T20b and T20 shipped inside the roster** (design.md and the parent were already counted). **T21 adds 2** (§10.23, C55): `services/cache/lru-evict.ts` and `lru-evict.test.ts`, whose `check-coverage.ts:318` comments the +2 shift falsified — both already in the union via Phases 2/3, so the union holds at 86 while the sum moves |
| **sum** | ~~83~~ → ~~86~~ → ~~92~~ → ~~101~~ → ~~105~~ → ~~107~~ → ~~109~~ → ~~110~~ → **112** | **32** | Phase 2 **+4** and Phase 3 **+11** since the 92; **Phase 5 +2 at T15** (§10.18); **Phase 6 +1 at T16/T17** (§10.19); **Phase 7 +2 at T21** (§10.23 — the two lru-evict files) |
...
→ ~~**12**~~ → ~~**17**~~ → ~~**18**~~ → ~~19~~ → **20 files in more than one phase, ~~four~~ → six of them in three
phases** (T16/T17, §10.19 — the twentieth is `docs/ONBOARDING.md`; the fifth and sixth three-phase
members are T21's two lru-evict files, §10.23), which
is what takes ~~83 to 78~~ → ~~86 to 80~~ → ~~92 to 80~~ → ~~101 to 81~~ → ~~105 to 84~~ →
~~107 to 86~~ → ~~109 to 86~~ → ~~110 to 86~~ → **112 to 86**. *(The `105 to 84` pair stood unstruck through T12–T14 while the sum
...
~~**15 files × 1 + 3 files × 2 = 21 = 107 − 86** at T12~~ → ~~**15 × 1 + 4 × 2 = 23 = 109 − 86** at T15~~ → ~~**16 × 1 + 4 × 2 = 24 = 110 − 86 at T16/T17**~~ → **14 × 1 + 6 × 2 = 26 = 112 − 86 at T21**, the fifteenth two-phase file at T12 being `spec.md`, which T8b
brought into Phase 2 and T11 brings into Phase 3 (**C68**); at T15 `check-tools-thin.test.ts` moves to the three-phase group (its fourth member) and `design.md` enters the two-phase group in its place (§10.18); at T16/T17 `docs/ONBOARDING.md` becomes the **sixteenth** two-phase file (Phases 6 and 7 — **C82**, §10.19); **at T21 `services/cache/lru-evict.ts` and `lru-evict.test.ts` both move from the two-phase group to the three-phase group** (Phases 2, 3 and now 7 — the C55 repoints, §10.23), taking it to **six** members, each step re-derived rather than assumed. **The identity is UNCHANGED by T12 and
...
| **T18** | **The 3 prose and fixture sites — R-32 as corrected to 3 (§3.5 item 2).** `CLAUDE.md:237`'s naming-trap note, `scripts/check-coverage.ts:281`'s docblock citation *(was `:279`; **+2** at T21's −4/+6 comment swap — C55, §10.23)*, and `scripts/__tests__/check-coverage.test.ts`'s **8** fixture citations at `:12`, `:39`, `:52`, `:59`, `:107`, `:108`, `:116`, `:144` *(seven shifted +1 by T21's `existsSync` import; grep-verified)*. The fixtures are synthetic (`BASE = "/repo/packages/core"`, never resolved on disk) so **no test breaks if they are missed** — they are repointed as prose. **Sequence after T21** — satisfied. **DONE — §10.24. Phase 6 CLOSES**: all 10 sites swapped in place, zero net line change, repo-wide residual outside `.specs/`/`.ua/` collapsed to `CHANGELOG.md:35` alone (the permanent C52 record). Single combined-lens critic (route deviation recorded in §10.24 with its reason): 8/8 verdicts confirmed, `graph-queries.ts` still 440 lines so the repointed docblock figure is exact. Its one finding — the `:237` trap note's two-way framing was measured three-way at Design and owned by nobody — is **assigned to T22** | 3 files | — |
...
| R-36 | **~~Nine~~ → Ten new source files must each clear the 90% per-file coverage floor independently, and `coverage` is in `main`'s live required checks** (**C84**, §10.21 — nine predates C33's T14b minting the tenth module, `acquire-indexing-lease.ts`; measured on the shipped tree: 1 cache + 6 file-read + 2 indexing + 1 project-identity) | R-23 as corrected: ~~nine~~ → ten, not five or six. `2ea4ebd` is the measured precedent for the red arriving at all. GMS-05 AC-2 forbids a new `EXCLUSIONS` entry — and T21 is *deleting* one in the same PR | T1–T3 write the highest-stakes tests **before** the gate can go red (R-26). The two `scripts/` files are outside the floor (`isMeasuredSource` returns `false`), so the gate script carries no incentive pressure |
...
**Applied at T20b — all twenty, in place (§10.21).** Every `design.md:NNN` in this table is the
frame its row was minted in (`d7091ac` for the Tasks-era rows, later task frames where a row says
so); the application moved `design.md` by **+76 net lines**, so current positions are found by
**content**, never by these numbers. The application's own citation sweep and its one cascade
(**C84**) are in §10.21.
...
### 10.21 T20b — executed, 2026-08-03

**All twenty §8.1 rows are applied into `design.md` in place — +103 insertions / −27 deletions, +76
net lines, every amendment carrying its C-number and §10.x pointer, verified row-by-row by an
independent critic: twenty of twenty APPLIED-CORRECT, zero missed sites, zero scope creep, six
inherited-defect spot-checks all clean. One new plan defect the application itself surfaced —
C84, the eighty-ninth: §8.1 row 19 adds table row 8c and no row carried the consequence that
`design.md`'s thrice-stated "Nine new files under the coverage floor" is thereby falsified. Measured
on the shipped tree: TEN (1 `services/cache/` + 6 `services/file-read/` + 2 `services/indexing/` +
1 `services/project-identity/`). Fixed in the same commit at all three `design.md` sites and at this
document's own R-36 — a correction's cascade caught before commit rather than three tasks later,
which is C65's shape avoided rather than repeated.**

#### The citation adjudication the +76-line shift demanded, on T15's §10.18 precedent

The red-team lens found the mechanism before the author did: T20b does to `design.md` exactly what
T15's +15 did to `ci.yml`, and the sweep population is *citations into `design.md` by line number*.
Enumerated repo-wide (tracked + `.specs/`, this feature's `design.md` only):

| citing site | classification |
| --- | --- |
| `HANDOFF.md:96` (`:930`, `:1042`), `:1047` (`:828`), `:1049` (`:970`) | **frame-stated records** — each describes what a past task found or did, in that task's tree (C52); none claims current-tree content |
| `STATE.md:665` (`:747`), `:783-784` (`:828`, `:970`) | **frame-stated records**, same class |
| this document — §1 Phase-5 row (`:828`), §6 item 8 (`:747`), §10.3 (`:930`, `:1042`), §10.15 (`:747`), §10.18's table (`:828`, `:970`) | **frame-stated records** — dated task records in their own frames |
| §8.1's own twenty rows | **the ledger** — frame-stated as a set by the line added under the table (found by content, never by number) |
| `~/prd-exec-instruments/t15-verify-post.ts` | **the one candidate LIVE sensor — and it survives by construction**: all three `design.md` assertions are content regexes (`ci.yml:200` runs it in `build``, `ci.yml:215`), no line-number anchoring; re-run post-edit, 16/16 green |
| tracked source files citing this `design.md` by line | **zero exist** (`git grep`) |

**Zero live repoints owed** — the entire population is records in their frames plus one
content-anchored instrument. The adjudication differs from T15's in outcome (T15 had two live
sites) and matches it in method; the difference is the population's character, not the rule.

#### The Plan Challenge gate on T20b — two modes on the APPLIED state, and the value audit ran second

Both critics ran read-only against the uncommitted working tree, SHA-256-snapshotted before and
verified unchanged after; no edit ran while either read. **Evidence audit (application fidelity):
20/20 APPLIED-CORRECT** against the ledger and the §10.x sources, membership swept in both
directions (every insertion tagged `§8.1 row N`, every row found), plus an unprompted sweep for
stray pre-correction values (`20 sites`, `9 fixture citations`, `five recommendations.push`, …) —
all remaining occurrences adjudicated as different subjects or already-corrected. Its named residual
is structural and true: T20b verifies transcription against `tasks.md`, so it is downstream of
T1–T19's own measurements — T25's re-derivation is the sensor for that, not another transcription
pass. **Red team: one critical, one high, one medium, one low — all four adopted**: the citation
sweep above (critical — mechanism confirmed, though its three "dead" sites re-classified as
frame-stated records once the C52 rule was applied to each, and the live-sensor candidate it did
not check, `t15-verify-post.ts`, was); C84 (high — confirmed by tree count, fixed as above); the
`RED 2 of 30` union cross-reference at §6.5 (medium — added); two soft-wrapped `~~` spans rejoined
onto single lines (low). Its escalation flag was answered by taking the two decisions it said were
unowned: both are author-level under standing rules (C52/C55 for the sweep's classifications; a
tree count for C84), on the C34/C35 precedent — re-decide only if you disagree.

#### Gates — a `.specs/`-only commit, T5's and T15's precedent

Write set is 2 files, both `.specs/` markdown (`design.md`, this document). `test:scripts`
**1115 pass / 0 fail across 49 files** + shell suites, exit 0 — the only battery member with a
measured path to sensitivity (`check-stale-pointers` runs there; `.specs/` is its `EXCLUDED` second
entry, re-confirmed). `check-tools-thin` **byte-identical** re-read (`PASS — 0 of 30 … 398
members`); `check-core-layering` **PASS — 0 across 982 edges / 920 files**, both unchanged from
T19's readings, as they must be — no tracked code file is touched. `t15-verify-post.ts` **16/16**
post-edit. lint / type-check / build / `bun run test` / `test:plugins` **N/A by measured
population** (markdown-only diff; T15's rule).

**Instruments**: none added (79 unchanged). The citation sweep reused `git grep` with the §10.21
...
### 10.23 T21 — executed, 2026-08-03

**RFS-05 AC-4 closes on an observed red taken FIRST: the new existence pin was written and run
against the still-dangling tree — `22 pass / 1 fail`, the new test the failure — and only then was
the entry deleted — `23 pass / 0 fail`, `EXCLUSIONS.length` 9 → 8 read by importing the module.
The gate itself is untouched beyond the sanctioned deletion (Design §4's decision: close the class
in the TEST); the assertion resolves `path.resolve(import.meta.dir, "..", "..")`, the same
absolute path as the gate's own `REPO_ROOT`, never the synthetic `BASE`, never cwd. Deletion is
behavior-neutral by consumption-site reading, not assumption: `EXCLUSIONS` is consumed as exact
`Set.has(relativePath)` (`check-coverage.ts:616`/`:624` region), and a path resolving to no file
can never appear as a key. All 8 survivors verified on disk — the deleted entry was the only
dangle. Zero new plan defects; running total stays eighty-nine.**

#### The write set grew 2 → 4 tracked files + 2 `.specs/`, under C55's standing rule

The −4/+6 comment swap shifts every `check-coverage.ts` line below it **+2**, and the `existsSync`
import shifts every `check-coverage.test.ts` line below `:21` **+1**. Live-citation sweep over
both filenames, every hit adjudicated: **four live sites repointed** —
`services/cache/lru-evict.ts:70` and `lru-evict.test.ts:173` (`:318` → **`:320`**, tracked source
comments), `design.md`'s docblock cite (`:279` → `:281`) and `isMeasuredSource` span (`:379-388` →
**`:381-390`**), plus `design.md`'s fenced-code cite of the pinning test (`:178` → **`:179`**) and
the eight fixture citations in `design.md` §1.4 and T18's row (seven +1, `:12` unshifted,
grep-verified). **Frame-stated and left, with the adjudication recorded so T25 does not re-flag
them**: `tasks.md:5140` (§10.19's residue table — a dated record of the T16/17 tree; its
"T18's" label assigns the underlying SITES, not the record), §10.7's two `:318` cites (T6's dated
record), and every `validation-pr2.md` / `sensor-repair` / `STATE.md` hit (other features' dated
artifacts; the `t15-verify-post.ts` precedent — records keep their frame's numbers on purpose).

#### The Plan Challenge gate on T21 — both lenses converged on the author's shift constant, and it was wrong

**The author's repoint arithmetic said +3; the true hunk is −4/+6 = +2.** Three consequences, all
caught before commit, all fixed: the two lru-evict comments were repointed to `:321` where the
original `:318` referent (`if (current && line.startsWith("DA:"))`) now sits at **`:320`** — the
author's own "content check" read `:321`'s `line.slice(3)` parse and semantic-matched the wrong
neighbor (*a field name is not its subject*, in line-number form: content-anchoring means finding
the line the OLD number pointed at — `git show HEAD:<file> | sed -n '318p'` — not a line about the
same topic); the `isMeasuredSource` span gained a phantom trailing line (`:391` is blank); and both
"+3" prose labels were arithmetically wrong even where the delivered number happened to be right
(`279 + 2 = 281`). **A fourth live site the sweep missed entirely** — `design.md:379`'s fenced-code
`:178` — was found by the evidence audit. The red team added two adopted lows: the existence pin is
**load-bearing in Linux CI only** for the case-drift shape (default APFS `existsSync` is
case-insensitive — probed true on this machine; now stated in the test's own comment), and the
pin still cannot tell a canonical file from a copy-not-move leftover (recorded residual; `git mv`
is this repo's convention). The audit's escalation — re-derive T15/T16-17's repoints the same way —
was **adjudicated rather than inherited**: those tasks' repoints were instrument-verified
(`t15-verify-post.ts` content regexes, re-run 16/16 this task; T17's AST rewriter + resolver
mirror), not hand-computed arithmetic, which is exactly the difference that made T21's the one
that broke. *Hand arithmetic under C55 is now the recorded anti-pattern: repoint by content
identity against `git show HEAD`, never by adding a delta.*

#### Gates

`lint` **0**. `test:scripts` **1116 pass / 0 fail across 49 files** (1115 + the new existence
pin), exit 0, twice — either side of the critic fixes. The touched core suite standalone:
`lru-evict.test.ts` **15/0/43** (comment-only diff). `check-tools-thin` **byte-identical**
(`0 of 30`, 398 members — `services/` and `scripts/` files are outside its population);
`check-core-layering` **PASS 982 / 920 unchanged** (comment-only edits move no edge).
`bun run test` / `test:plugins` / type-check / build **N/A by measured population** (no
`packages/*` production logic changed; the two core files' diffs are inside comments — verified by
the suite runs above). Host load 2.20 at the readings.

**Instruments**: none added (79 unchanged).

**Running total: eighty-nine plan defects — unchanged at T21.**

### 10.24 T18 — executed, 2026-08-03

**Phase 6 is CLOSED: the rename's 3 prose/fixture carriers are repointed — `services/graph/` →
`services/memory-graph/` at exactly the row's 10 sites (CLAUDE.md:237; check-coverage.ts:281;
the test file's 8 fixture citations at their T21-shifted positions), in place, zero net line
change in all three files, so no repoint cascade. Post-swap the repo-wide literal residual
outside `.specs/`/`.ua/` is exactly `CHANGELOG.md:35` — §10.19's enumerated set collapsed to its
one permanent member. Zero new plan defects; running total stays eighty-nine.**

**Route note, recorded because it deviates**: T18 ran a **single combined-lens critic** rather
than the feature's two-critic norm — 10 literal swaps, zero line shifts, zero arithmetic, no
published surface; the packet carried both lens's surfaces and the §10.23 precedent of what
two critics buy. All 8 verdicts CONFIRMED (byte-exact diff; residual = CHANGELOG.md:35 alone;
suite 23p/0f; the renamed directory real on disk with `git mv`-consistent link counts; no
concatenated/encoded literal carriers; `graph-queries.ts` still **440 lines**, so the repointed
docblock figure is exact, not freshly stale). Its one substantive finding is a **pre-existing
scope gap, not a T18 defect, and it now has an owner**: CLAUDE.md:237's *"Two names are a trap"*
framing was measured **three-way** at Design (`design.md` §1.1 — `data/symbol/` is the third
directory, and the one that already produced a real path error) and no task owned widening the
prose. **Assigned to T22** (already editing CLAUDE.md; its row gains the item under C55/C62's
owner rule). The docblock-repoint class question the packet posed was adjudicated: the test
file's `:12` note is a LIVE fixture explanation, not a C52 dated record, so tracking the current
path is correct — and its measured content survives a `git mv` byte-identically.

#### Gates

`lint` **0**. `test:scripts` **1116/0 across 49**, exit 0. The touched suite standalone
**23p/0f**. `check-stale-pointers` `PASS — 0 broken, historical exactly at its pin of 28` —
the nothing-else-broke frame ONLY (stems blind to these basenames, §10.19's rule).
`check-tools-thin` / `check-core-layering` **N/A by population** (no `packages/core/src` file
touched; CLAUDE.md and `scripts/` are outside both). `bun run test` / `test:plugins` /
type-check / build **N/A by measured population** (prose and synthetic-fixture strings only;
the one code-adjacent site, `isMeasuredSource(...)` at `:144`, is string-shape-only and
asserted green in the suite run).

**Instruments**: none added (79 unchanged).

**Running total: eighty-nine plan defects — unchanged at T18.**

### 10.25 T22 — executed, 2026-08-03

**RFS-05 AC-5 closes, plus the T18-assigned widen: CLAUDE.md's "24 migrations" → 23 with the
metric named (23 `migration.sql` directories; the 24th tracked entry is `migration_lock.toml`),
its publish-rehearsal "five skipping lines" → one line per publishable package (eight today from
every non-`private` `package.json`; five when v1.3.1 shipped — the critic verified BOTH frames
against `git ls-tree v1.3.1`, 5 exact), the naming-trap note widened to the three-way trap
`design.md` §1.1 measured (with the real `data/symbol/` path error named), and ONBOARDING's
deferral marker re-scoped to "its own follow-up change after PR-D merges" so it stays true once
the PR has. Write set 2 files, zero growth. Zero new plan defects; running total stays
eighty-nine.**

**Single combined-lens critic (T18's recorded route, same reason — premeasured doc figures, no
code): all five quantitative claims REPRODUCED exactly** — 23/24 split, the 8 publishable
enumerated by name, `data/symbol/symbol-repository-pg.ts` real with the wave-5 citation error
reproducing at `context.md:16`/`tasks.md:84,191`, v1.3.1's 5, and the zero live
`CLAUDE.md:NNN`/`ONBOARDING.md:NNN` citations re-swept (+7/+1 shifts land on dated `.specs`
records only). **Two of its findings were the author's own staged prose, both fixed pre-commit**:
the grafted sentence read *"Two names are a trap … and the trap is three directories, not two"* —
self-contradictory in one breath (lead-in re-written to "Naming is a trap … three directories");
and "shipped docs" over-labeled internal specs (→ "in-repo specs").

**`test:scripts` exited 1 on its second run and it is not T22's — recorded with all three
readings.** Run 1 (post-edit): exit 0, 1116/0. Run 2 (post the two prose fixes): exit 1 — the
shell suite `test-setup-wizard-db-selection.sh` read `not ok — migrations fail closed`, 10p/1f.
Standalone re-run: **11p/0f exit 0**; full aggregate re-run: **1116/0 + 11p/0f, exit 0**. The
case is `assert_joined_contains` — a **static string assertion** over
`scripts/setup-local-first.sh` (`bunx prisma migrate deploy \|\| die`), a file **zero-diff across
all three runs and untouched by T22** (whose whole diff is prose in two docs), so the red was the
shell harness's own plumbing, not the subject; host load 2.20–2.78 throughout, not a loaded-host
artifact. Both readings kept on the T3/T7/T9 precedent.

#### Gates

`test:scripts` **1116/0 across 49** + shell suites, exit 0 (twice green, the middle red
adjudicated above). `check-stale-pointers` `PASS — 0 broken / pin 28` (frame).
`lint` / structural gates / `bun run test` / `test:plugins` / type-check / build **N/A by
measured population** (two markdown files; oxlint does not lint markdown; no `packages/` or
`scripts/` code touched).

**Instruments**: none added (79 unchanged).

**Running total: eighty-nine plan defects — unchanged at T22.**

### 10.26 T23 — executed, 2026-08-03

**RFS-05 AC-1's state files: `FEATURES.json` PR-D → `status: complete`, `phases.execute: true`
(both flips position-checked and the whole file re-parsed valid); the parent
`core-layering-god-module-split` → `complete`, its notes' *"PR-D … in Specify"* — the exact
stale-field class AC-1 polices, §3.5 item 9's prediction landing a third time — corrected in the
same edit. Both notes tails now state the honest frame: **T25 independent validation (different
author) is still owed before merge; the status is the T23 state-file record, not the validation
verdict.** `active_feature` verified `skills-directive-dedup` and untouched (fourth check).
`HANDOFF.md`/`STATE.md` carry the completion state via this task's own commit. PR-D's notes also
had their pre-T8b figures corrected (28 rows / 78 files → 29 / final sum 112, union 86) rather
than appended-around — a status note that contradicts its own tail is C19's shape. Staged
explicitly; `.specs/reports/` stays untracked. Zero new plan defects; running total stays
eighty-nine. One process note: a `_status_note` field was briefly added to the parent entry and
reverted before staging — inventing a schema field on a machine-consumed file is the wrong tool;
the notes string is the record.**

Gates: `test:scripts` sensitivity for `FEATURES.json` — the T5-measured population holds (no
suite reads it as data); run anyway post-stage with the docs commit's battery. Structural gates
N/A by population.
