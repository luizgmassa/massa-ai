...
| `MASSA_AI_READ_FILE_MAX_LINES` / `MASSA_AI_READ_FILE_ROOTS` absent from `turbo.json`'s 24-entry `passThroughEnv` | AD-010 says adding a `MASSA_AI_*` knob means editing that list | **No failure observed today**, ~~because both read-file suites set the var in-process (`read-file-containment.test.ts:180`, `read-file.test.ts:44`)~~ → **amended at T11 (C68, `tasks.md` §10.13): the two cited sites set `MASSA_AI_READ_FILE_ROOTS`, not `MASSA_AI_READ_FILE_MAX_LINES`**, and the row's own subject names both variables, so the sentence was read as covering a variable neither site touches. Corrected per variable: **`ROOTS`** is read at CALL time and both cited suites do set it in-process; **`MAX_LINES`** is a module-level `const` evaluated once at import, so no in-process mutation can reach it and **no test in the repo exercises its override at all** — the only site that sets it, `apps/tools-api/src/routes/workspace.test.ts:422`, drives a *third* copy of the same read. Still a convention gap rather than a live defect, and recorded as such rather than fixed under a criterion it does not fail |
| the N9 line cap is read from `MASSA_AI_READ_FILE_MAX_LINES` at **three** independent sites | `services/file-read/line-range.ts` (module-level, once per process — T11's; `tools/read_file.ts:33-36` before that), `apps/tools-api/src/routes/workspace.ts:774` (**per request**, inside the `symbol_snippet` handler) and `apps/mcp-client/src/embedded-api-client.ts:327` | **Added at T11 (C68)** — the row above priced this table at two sites and the third is why its own evidence read wrong: `workspace.test.ts:422` sets the variable and the test passes, which looks like proof the tool honours it. The three differ in *when* they read, not only in *where*, so they are not merge candidates without deciding which timing is correct. Out of scope on R-07's ground: a behavior question inside a behavior-preserving PR |
