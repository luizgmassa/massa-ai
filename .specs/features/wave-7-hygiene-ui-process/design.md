...
graph TD
    ...
    subgraph "P3 Cleanup"
        ...
        M22[M22: hook breadcrumb] --> Hook[massa-th0th-hook.ts]
    end
```
...
| `config.embedding` | `packages/shared/src/config/massa-th0th-config.ts:197` | Align health-checker to read this, not hardcode |
| `postObservation` | `apps/claude-plugin/hooks/massa-th0th-hook.ts:136` | Add breadcrumb timing inside this fn |
...
**Sandbox default: `auto`** (not `on`). `auto` uses the sandbox if the platform tool is available (Docker on Linux, `sandbox-exec` on macOS), falls back to current best-effort containment if not. `MASSA_TH0TH_EXECUTOR_SANDBOX=none` forces best-effort. `MASSA_TH0TH_EXECUTOR_SANDBOX=on` forces sandbox and errors if unavailable. This prevents F1 (CI can't run Docker).
...
- **Dependencies**: `MASSA_TH0TH_WEB_WRITE_MODE=true` env gate (default off)
...
- **Location**: `apps/claude-plugin/hooks/massa-th0th-hook.ts:136` (`postObservation`)
...
| --- | --- | --- |
| Docker not available (Linux) | Teaching error: "Docker not found; set MASSA_TH0TH_EXECUTOR_SANDBOX=none" | Execute fails with clear message |
...
| --- | --- | --- | --- |
| Sandbox startup latency | executor.ts:449 | Docker spawn ~1-2s adds latency to every execute call | Use `sandbox-exec` on macOS (fast), Docker only on Linux; `MASSA_TH0TH_EXECUTOR_SANDBOX=none` opt-out for dev |
