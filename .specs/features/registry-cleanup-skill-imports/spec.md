...
| `validate_skill.py` stays Python | SUPERSEDED (user, 2026-08-05, post-validation): ported to `scripts/validate_skill.ts` (Bun + `Bun.YAML`); the `.py` is not shipped; workflow invokes `bun scripts/validate_skill.ts` with a no-code-execution manual-checklist fallback | Repo runtime is Bun; a Python dependency was the only foreign-runtime script in the skill surface | y (user, 2026-08-05) |
