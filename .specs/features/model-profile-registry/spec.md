...
| --- | --- | --- |
| **MPR-R1** | A single registry file is the only hand-authored place naming a model or effort level for any agent or workflow on any host. | A scripted scan for model tokens returns **0** hits in charter sources, mirrored charters, generated agent bodies, and the generator. **One intentional exception**, declared here rather than hidden: the frozen-baseline test in `subagent-parity.test.ts` hard-codes the three before/after model pairs. That is the anti-tautology anchor — a fixture test derived entirely from the registry would pass whatever the registry contains, so the literal is what gives it teeth. Effort enums are imported from the resolver, not restated. |
