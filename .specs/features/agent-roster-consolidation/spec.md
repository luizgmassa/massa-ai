...
| A20 tests-lens audit agent | `tests-audit`, `implementation-audit` (Tests lens), and `pr-review` (row 5) dispatch `test-engineer` in `audit` mode — `standard` tier, `write` permission — where they used the `deep`, read-only audit-specialist `tests` lens. **Accepted risk:** audit mode loses tool-level read-only enforcement and deep-tier reasoning; the write-nothing rule for audit mode is charter prose only | One charter has one permission and one tier; Dispatch AC-3 names `test-engineer` for the tests audit | n — recorded (roster-swap review A4) |
| A21 Model tiers removed upstream | `model-catalog-revamp` (merged from `main` in `ec21d05e`) removed `metadata.model_tier`; the 7 charters carry none. Built-in profiles override exactly `builder`, `designer`, `test-engineer`; `code-explorer`, `code-reviewer`, `judge`, `product-manager` resolve to the profile default (the strongest model). The retired `documentation-agent` overrides were dropped. Design C1's Tier column is historical | Upstream contract, adapted after merge | n — recorded (orchestrator, 2026-09-23) |
...
7. No tracked file outside `.specs/`, `.ua/`, and `CHANGELOG.md` history SHALL reference `planner`, `context-curator`, `documentation-agent`, `investigator`, `navigator`, `meta-judge`, `plan-critic`, `furps-analyst`, `requirements-analyst`, `verification-agent`, `mobile-specialist`, `architecture-specialist`, `audit-specialist`, or `reviewer` as an agent name, except a single old→new mapping table in `skills/AGENTS.md`, the historical record in `docs/removed-features.md` (DOC AC-2), and historical review-finding ids such as `plan-critic F3` in comments. *(amended 2026-09-23 by the orchestrator after independent validation: DOC AC-2 requires a second record of the old names)*
...
1. The workflows `architecture-audit`, `bugs-audit`, `implementation-audit`, `code-quality-audit`, `security-audit` SHALL dispatch `code-reviewer` (with the matching lens) for the correctness, architecture, code-quality, and security lenses (the `performance` lens is dispatched only where a workflow requests it, today `pr-review`), *(amended 2026-09-23 by the orchestrator after independent validation: none of the five audits dispatched a performance lens before or after this change)* and `implementation-audit` SHALL dispatch `product-manager` (`audit` mode) for its Requirements lens and `test-engineer` (`audit` mode) for its Tests lens.
...
5. The Plan Challenge policy sentence SHALL name `spec-driven`, `feature`, `create-adr`, `create-rfc`, `create-tdd`, `refactor`, and each named file SHALL exist and contain "Plan Challenge Gate" or "Plan Challenge lite gate". *(amended 2026-09-23 by the orchestrator after independent validation: `feature.md` uses the lite-gate wording and is untouched by this feature)*
...
7. ~~The web UI `WORKFLOW_STEMS` list SHALL equal the 36 current stems.~~ *Superseded 2026-09-23 (orchestrator, merge `ec21d05e`): `model-catalog-revamp` on `main` removed the Workflow Tiers picker and `WORKFLOW_STEMS` with it; the web UI no longer lists workflow stems.*
