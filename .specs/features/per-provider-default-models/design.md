...
| R-04 | `qwen3-vl:8b` is vision-language, serving 7 text-only judgment sites through `generateObject` with `json_schema`. | It is non-thinking instruct, the repository's stated hard requirement. Two further facts the draft missed: `inference-providers.ts:38-45` records that the `/v1/responses` vs `/v1/chat/completions` finding was measured against a **different** model, and `config/index.ts:742-744` calls the reasoning-channel fallback "dormant" *because* the default is pure-instruct — a comment this swap invalidates. Execute must exercise a real structured call on both providers. |
