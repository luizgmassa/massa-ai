...
| A-04 — **CORRECTED by measurement.** The instruct model is vision-language *and* the Ollama build is thinking-capable | Accept anyway | The original wording — "it is a non-thinking instruct model, which is the repository's stated hard requirement" — is **false for the Ollama build**. Measured against a `json_schema` request on `/v1/chat/completions`: `qwen3-vl:8b` returns a reasoning channel **even with the `think: false` the seam injects** (`injectsDisableThink: true` → `_wrapFetchDisableThink`), which only halved the latency, 4.7 s → 2.7 s. What does **not** reproduce is the failure mode `CLAUDE.md` warns about: structured output landed correctly in `content` in 2.7 s, nowhere near the 90 s timeout. The LM Studio build is clean — `qwen3-vl-8b-instruct` returned no reasoning channel at all in 2.0 s, with no suppression sent (`injectsDisableThink: false`). Same model family, different builds, different behaviour. The vision head is unused by all 7 NL-judgment sites. | y — with the classification corrected and the risk measured rather than assumed |
