...
| --- | --- |
| CLA-01,02 | `scripts/generate-subagent-artifacts.ts` Claude emitter → `apps/claude-plugin/agents/massa-th0th-*.md`; `apps/claude-plugin/install.sh` copy loop |
...
| CLA-05 | `apps/claude-plugin/install.sh --uninstall` removes only `massa-th0th-*.md` (name-prefix scoping) |
...
| CDX-01..06 | Codex emitter → `apps/codex-plugin/agents/massa-th0th-*.toml`; `apps/codex-plugin/install.sh` writes to `~/.codex/agents/` (OUTSIDE plugin dir); idempotent; uninstall by `# massa-th0th-owned` marker |
| CDX-07 | Codex emitter escapes `"""` → `\"\"\"` in `developer_instructions`; TOML round-trip test; `# massa-th0th-owned` top comment |
...
| CRS-01..05 | Cursor emitter → `apps/cursor-plugin/agents/massa-th0th-*.md`; `apps/cursor-plugin/install.sh` copies into plugin `agents/`; navigator preserved; uninstall removes plugin dir (unchanged) |
...
| OPC-01..06 | OpenCode emitter → `apps/opencode-plugin/agents/massa-th0th-*.md`; new `install-agents` subcommand on `massa-th0th-config` CLI writes to `~/.config/opencode/agents/` or `.opencode/agents/`; idempotent; uninstall by `metadata: { massa-th0th-owned: true }` marker |
| OPC-07 | OpenCode emitter frontmatter `metadata: { massa-th0th-owned: true }` + per-agent `permission` (bash deny/ask/allow per mapping) |
...
- `apps/claude-plugin/install.sh:181-212` — current install copies `commands/*.md` + `agents/massa-th0th-navigator.md` + merges hooks into `settings.json`. **No subagent loop exists yet.** The agent copy is a single `cp` at `:192`.
- `apps/claude-plugin/agents/massa-th0th-navigator.md:1-29` — frontmatter uses JSON-array `tools: ["mcp__massa-th0th__*", "Read", "Grep", "Glob", "Bash(pwd)"]` (array form, the convention CLA emitters follow).
...
- `apps/codex-plugin/install.sh:170-213` — copies plugin bundle to `~/.codex/plugins/massa-th0th/` + merges hooks into `~/.codex/hooks.json`. **No agent writing** (Codex agents live in `~/.codex/agents/`, separate).
...
- `apps/opencode-plugin/package.json:1-47` — npm package `@massa-th0th/opencode-plugin@1.1.0`, `bin.massa-th0th-config: dist/config-cli.js`. **Agents are NOT npm-shipped** (`files: ["dist"]` only) — OpenCode discovers agents from the filesystem (`~/.config/opencode/agents/`).
...
graph TD
    ...
    GEN -->|Claude emit| CLA["apps/claude-plugin/agents/massa-th0th-*.md"]
    GEN -->|Codex emit| CDX["apps/codex-plugin/agents/massa-th0th-*.toml"]
    GEN -->|Cursor emit| CRS["apps/cursor-plugin/agents/massa-th0th-*.md"]
    GEN -->|OpenCode emit| OPC["apps/opencode-plugin/agents/massa-th0th-*.md"]

    subgraph "Installers (existing, extended)"
      CLA_INSTALL["apps/claude-plugin/install.sh"] -->|copy| CLA_TARGET["~/.claude/agents/massa-th0th-*.md"]
      CDX_INSTALL["apps/codex-plugin/install.sh"] -->|write| CDX_TARGET["~/.codex/agents/massa-th0th-*.toml<br/>(OUTSIDE plugin dir)"]
      CRS_INSTALL["apps/cursor-plugin/install.sh"] -->|copy into plugin dir| CRS_TARGET["~/.cursor/plugins/massa-th0th/agents/massa-th0th-*.md"]
      OPC_CONFIG["massa-th0th-config agents"] -->|write| OPC_TARGET["~/.config/opencode/agents/massa-th0th-*.md<br/>(OUTSIDE npm package)"]
    end
...
| --- | --- | --- |
| Charter `SKILL.md` files | `skills/*/SKILL.md` (12) | Generator reads frontmatter (`name`, `description`, `metadata.model_hint`, `metadata.permission`) + body (Mission/Responsibilities/Restrictions/Inputs/Outputs/Invocation/massa-th0th Integration) as the single source |
| `massa-th0th-navigator.md` | `apps/claude-plugin/agents/`, `apps/cursor-plugin/agents/` | Preserve as-is (Claude/Cursor). NOT generated — it's an index-first agent distinct from the 12 specialists. Codex/OpenCode do not receive it. |
| `install.sh` copy loops | `apps/claude-plugin/install.sh:184-192`, `apps/cursor-plugin/install.sh:188-204` | Extend the agents copy from single `cp` to a loop over `massa-th0th-*.md` (Claude/Cursor) |
...
| Ownership-marker convention | `scripts/install-agents.ts:41` (`OWNED_MARKER = "_massaTh0thOwned"`) | Codex uses `# massa-th0th-owned` top comment; OpenCode uses `metadata: { massa-th0th-owned: true }` frontmatter (hosts ignore unknown fields) |
| `massa-th0th-config` CLI | `apps/opencode-plugin/src/config-cli.ts:62-194` | Add `agents` subcommand (`agents install [--user|--project]` / `agents uninstall`) — reuses the CLI entrypoint |
...
| --- | --- |
| Claude Code agent discovery | Files at `~/.claude/agents/massa-th0th-*.md` (user) or `.claude/agents/` (project); frontmatter `name`/`description`/`tools`/`model`/`effort` |
| Codex agent discovery | Files at `~/.codex/agents/massa-th0th-*.toml` (user) or `.codex/agents/` (project); TOML `name`/`description`/`developer_instructions`/`model`/`model_reasoning_effort`/`sandbox_mode` |
| Cursor agent discovery | Files inside plugin dir `~/.cursor/plugins/massa-th0th/agents/massa-th0th-*.md` (auto-discovered); same frontmatter as Claude |
| OpenCode agent discovery | Files at `~/.config/opencode/agents/massa-th0th-*.md` (user) or `.opencode/agents/` (project); frontmatter `description`/`mode: subagent`/`model`/`reasoningEffort`/`permission`/`metadata` |
...
- **Claude emitter**: `name: massa-th0th-<charter-name>`, `description: <charter description>`, `tools: [...]` (read-only → `["Read","Grep","Glob","Bash"]`; write → + `Write`,`Edit`), `model: <claude alias>` (haiku/sonnet/opus), `effort: high`. Omits `hooks`/`mcpServers`/`permissionMode`. Body = charter body (Mission…Memory Boundary) verbatim.
- **Codex emitter**: TOML file. Top comment `# massa-th0th-owned`. `name = "massa-th0th-<name>"`, `description = "..."`, `model = "<codex id>"`, `model_reasoning_effort = "high"`, `sandbox_mode = "read-only"` (read-only) or `"workspace-write"` (write), `developer_instructions = """<escaped body>"""`. Escape `"""` → `\"\"\"` in body.
...
- **OpenCode emitter**: `description: <charter description>`, `mode: subagent`, `model: <charter hint verbatim>`, `reasoningEffort: max`, `permission: {edit: deny/allow, bash: deny/ask/allow}`, `metadata: {massa-th0th-owned: true}`. Body = charter body verbatim.
...
- **Purpose**: Copy the 12 generated `massa-th0th-*.md` into `~/.claude/agents/` (or `.claude/agents/`) alongside the existing navigator copy + hooks merge.
- **Location**: `apps/claude-plugin/install.sh:191` (after navigator `cp`)
- **Interfaces**: add loop `for src in "$SCRIPT_DIR/agents/"massa-th0th-*.md; do cp ...; done`; uninstall loop removes `~/.claude/agents/massa-th0th-*.md` (name-prefix scoping, preserves navigator which is `massa-th0th-navigator.md` — wait, that matches the prefix; **see Risks R1**)
- **Dependencies**: generated `apps/claude-plugin/agents/massa-th0th-*.md`
...
- **Interfaces**: `AGENTS_DIR="$CODEX_DIR/agents"`; loop `for src in "$SCRIPT_DIR/agents/"*.toml; do cp "$src" "$AGENTS_DIR/$(basename $src)"; done`; `--uninstall` removes only files with the `# massa-th0th-owned` top comment (grep + rm)
- **Dependencies**: generated `apps/codex-plugin/agents/massa-th0th-*.toml`
- **Reuses**: existing `CODEX_DIR` resolution

### `apps/cursor-plugin/install.sh` (EXTEND)

- **Purpose**: Copy the 12 generated `massa-th0th-*.md` into the plugin's `agents/` dir (alongside navigator).
...
- **Dependencies**: generated `apps/cursor-plugin/agents/massa-th0th-*.md`
...
- **Purpose**: New `agents` subcommand on `massa-th0th-config` writes the 12 generated `.md` files to `~/.config/opencode/agents/` (or `.opencode/agents/`).
- **Location**: `apps/opencode-plugin/src/config-cli.ts` (add `case "agents"`); generated files at `apps/opencode-plugin/agents/massa-th0th-*.md` (shipped in the npm package via `files` array update — **see Risks R2**)
- **Interfaces**: `massa-th0th-config agents install [--user|--project]` / `massa-th0th-config agents uninstall`; idempotent; uninstall removes only files with `metadata: { massa-th0th-owned: true }`
- **Dependencies**: generated `apps/opencode-plugin/agents/massa-th0th-*.md`
...
| User has unmarked `massa-th0th-<name>.md` (Claude) | Overwrite (name-prefix scoping) — **see Risks R1** | Pre-existing file replaced (acceptable; prefix is massa-th0th-owned namespace) |
| User has unmarked agent in `~/.codex/agents/` | Installer only writes `massa-th0th-*.toml`; user files preserved | None |
| `massa-th0th-config agents uninstall` finds no owned files | No-op, print message | None |
...
| --- | --- | --- | --- |
| R1: Claude uninstall `massa-th0th-*.md` glob also matches `massa-th0th-navigator.md` | `apps/claude-plugin/install.sh` (proposed uninstall loop) | Navigator would be removed on `--uninstall`, contradicting CLA-05 ("preserving navigator") | Uninstall loop excludes `massa-th0th-navigator.md` explicitly (`for f in massa-th0th-*.md; do [[ "$f" == *navigator* ]] && continue; rm ...; done`) OR the 12 specialists use a distinct prefix like `massa-th0th-agent-*.md`. **Design choice: exclude navigator by name in the uninstall loop** (keeps the `massa-th0th-` namespace consistent with Codex/OpenCode). Test asserts navigator survives uninstall. |
| R2: OpenCode agents shipped in npm package | `apps/opencode-plugin/package.json:10` (`files: ["dist"]`) | The generated `.md` agent files aren't in `dist`, so `npm install` won't include them → `massa-th0th-config agents install` can't find them at runtime | Two options: (a) add `agents/*.md` to `files` array so they ship in the npm tarball, and `massa-th0th-config agents install` reads from the package dir; (b) ship a separate `@massa-th0th/opencode-agents` package. **Design choice: (a)** — add `agents/` to `files`, generator output ships in the package, CLI reads from `__dirname/../agents/` (resolved at build time). |
| R3: Codex/OpenCode agent dirs are shared with user agents | `~/.codex/agents/`, `~/.config/opencode/agents/` | Uninstall must not remove user agents | In-file ownership markers (`# massa-th0th-owned` / `metadata: { massa-th0th-owned: true }`); uninstall greps for the marker. Test pre-seeds a user agent + runs uninstall + asserts user agent survives. |
...
| R10: `massa-th0th-navigator.md` uses `mcp__massa-th0th__*` in tools — the 12 specialists don't | `apps/claude-plugin/agents/massa-th0th-navigator.md:4` | Inconsistent tools surface between navigator and specialists | Acceptable: navigator is index-first (needs MCP tools); specialists are general-purpose (Read/Grep/Glob/Bash/Write/Edit). Different roles, different tools. |
...
| CLA-10 (Claude model+effort pin) | Parity test parses each `apps/claude-plugin/agents/massa-th0th-*.md` frontmatter, asserts `model` matches `AGENT_MODELS_CLAUDE[name]` and `effort: high` |
...
| CDX-07 (TOML round-trip + owned marker) | Parity test parses each `.toml` (TOML parser), asserts no parse error + first line is `# massa-th0th-owned` |
| CLA-05 (uninstall preserves navigator) | Install test: run `--uninstall`, assert `massa-th0th-navigator.md` survives, 12 specialists gone |
...
| Claude uninstall scoping | Exclude `massa-th0th-navigator.md` by name in the loop | R1: the `massa-th0th-*.md` glob would catch navigator; explicit exclusion preserves it per CLA-05 |
| Codex/OpenCode ownership marker | In-file (`# massa-th0th-owned` / `metadata`) | R3: shared agent dirs need a per-file marker for scoped uninstall |
