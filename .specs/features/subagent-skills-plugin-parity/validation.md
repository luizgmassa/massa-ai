...
| T5: Codex installer (TOML outside plugin dir, owned marker, idempotent) | ✅ Done | `install.sh:229-237` writes 12 TOML to `~/.codex/agents/`; `:170-176` removes only `# massa-th0th-owned` files. |
...
| T7: OpenCode `agents` subcommand | ✅ Done | `config-cli.ts:201-260` install/uninstall; writes 12 `.md` to `~/.config/opencode/agents/`; removes only `massa-th0th-owned: true`. |
...
| --------- | -------------------- | ---------------------- | ------ |
| CLA-01: 12 agent files at `~/.claude/agents/massa-th0th-<name>.md` with `name`/`description`/`tools`/`model` | exactly 12 files with the 12 registry names + 4 frontmatter fields | `apps/claude-plugin/__tests__/install.test.ts:242-259` — `expect(await pathExists(path.join(tmp, '.claude/agents/massa-th0th-${name}.md'))).toBe(true)` for all 12 names; parity `scripts/__tests__/subagent-parity.test.ts:193-198` parses `name`/`description`/`tools`/`model` frontmatter | ✅ PASS |
...
| CLA-08: no shipped name collides with Claude built-ins (`Explore`/`Plan`/`general-purpose`) | no `massa-th0th-<name>` ∈ builtins | `scripts/__tests__/subagent-parity.test.ts:178-187` — `expect(builtins.has(name)).toBe(false)` + `expect(builtins.has('massa-th0th-${name}')).toBe(false)` for Claude set | ✅ PASS |
...
| Criterion | Spec-defined outcome | `file:line` + assertion | Result |
| --------- | -------------------- | ---------------------- | ------ |
| CDX-01: 12 TOML files at `~/.codex/agents/massa-th0th-<name>.toml` with `name`/`description`/`developer_instructions`/`sandbox_mode` | exactly 12 TOML OUTSIDE plugin dir | `apps/codex-plugin/__tests__/install.test.ts:212-224` — `pathExists(...massa-th0th-${name}.toml).toBe(true)` for 12; `expect(agentsDir).not.toContain("plugins")`; parity `:230-267` parses `name`/`developer_instructions`/`sandbox_mode` | ✅ PASS |
...
| CDX-07: TOML round-trips (`"""` escaped) + `# massa-th0th-owned` top comment | parses cleanly + first line marker | `scripts/__tests__/subagent-parity.test.ts:255-266` — `expect(firstLine).toBe("# massa-th0th-owned")` + `toml.parse(raw)` succeeds; `apps/codex-plugin/__tests__/install.test.ts:230-239` mirrors it post-install | ✅ PASS |
...
| CDX-09: no shipped name collides with Codex built-ins (`default`/`worker`/`explorer`) | no name ∈ builtins | `scripts/__tests__/subagent-parity.test.ts:178-187` — `builtins.has(name)` + `builtins.has('massa-th0th-${name}')` false for codex set | ✅ PASS |
...
| Criterion | Spec-defined outcome | `file:line` + assertion | Result |
| --------- | -------------------- | ---------------------- | ------ |
| CRS-01: 12 agent files in plugin `agents/` dir with `name`/`description`/`tools`/`model` | 12 + navigator (13) in plugin agents/ | `apps/cursor-plugin/__tests__/install.test.ts:213-228` — `pathExists(...massa-th0th-${name}.md).toBe(true)` for 12 + navigator; `expect(files.length).toBe(13)`; parity `:158-166` exact-12 check | ✅ PASS |
...
| CRS-04: navigator remains in plugin `agents/` dir | navigator exists alongside 12 | `apps/cursor-plugin/__tests__/install.test.ts:219` — `pathExists(...massa-th0th-navigator.md).toBe(true)`; `apps/cursor-plugin/__tests__/manifest.test.ts:76-82` asserts navigator exists | ✅ PASS |
| CRS-05: `--uninstall` removes plugin dir (unchanged behavior) | plugin dir gone | `apps/cursor-plugin/__tests__/install.test.ts:234-244` — `pathExists(...cursor/plugins/massa-th0th).toBe(false)` after uninstall | ✅ PASS |
...
| Criterion | Spec-defined outcome | `file:line` + assertion | Result |
| --------- | -------------------- | ---------------------- | ------ |
| OPC-01: 12 `.md` at `~/.config/opencode/agents/massa-th0th-<name>.md` with `description`/`mode: subagent`/`model`/`permission` | exactly 12 files with 4 fields | `apps/opencode-plugin/src/__tests__/agents-install.test.ts:86-98` — `pathExists(...massa-th0th-${name}.md).toBe(true)` for 12; `:104-118` asserts `mode: subagent`; parity `:292-313` parses all frontmatter | ✅ PASS |
...
| OPC-07: `metadata: { massa-th0th-owned: true }` + per-agent bash mapping (deny/ask/allow) | marker present + bash per mapping | `scripts/__tests__/subagent-parity.test.ts:292-312` — `expect(fm.metadata).toContain("massa-th0th-owned: true")` + planner `bash: { "*": "ask" }`, strict `bash: deny`, write `bash: allow`; `apps/opencode-plugin/src/__tests__/agents-install.test.ts:104-118` mirrors marker | ✅ PASS |
...
| 3 | `apps/codex-plugin/install.sh:172` | Changed grep pattern from `^# massa-th0th-owned$` → `^# massa-th0th$` (breaks owned-marker matching). Ran `bun test apps/codex-plugin/__tests__/install.test.ts`. | ✅ Killed — 1 test failed: `install.test.ts:259` (owned TOML `pathExists` expected false, got true — files not removed because grep no longer matched the marker). 10 pass / 1 fail. |
...
- [x] existing `massa-th0th-<name>` without marker → backed up before overwrite: Codex/OpenCode use in-file marker (not name-based); install overwrites owned files identically (idempotent). ⚠️ Backup-before-overwrite convention (`.massa-th0th.bak-<ts>`) is NOT applied to agent files — only to `settings.json`/`hooks.json`. Spec edge case says "back it up before overwriting" for unmarked files; the installers overwrite owned files without backup. This is acceptable for idempotent owned files (identical content), but a truly unmarked `massa-th0th-<name>.md` (pre-existing, not from this feature) would be overwritten without backup on Claude (name-prefix scoping) — see design R1. Minor gap, low risk (massa-th0th- namespace is owned).
...
- Owned markers: `# massa-th0th-owned` (Codex), `metadata: { massa-th0th-owned: true }` (OpenCode), name-prefix (Claude/Cursor).
