...
| Criterion | Spec-defined outcome | `file:line` + assertion | Result |
| --------- | -------------------- | ---------------------- | ------ |
| OPC-01: 12 `.md` at `~/.config/opencode/agents/massa-ai-<name>.md` with `description`/`mode: all`/`model`/`permission` | exactly 12 files with 4 fields; `mode: all` so the Tab switcher lists them; `model` a resolvable `provider/model-id` | `apps/opencode-plugin/src/__tests__/agents-install.test.ts:86-98` — `pathExists(...massa-ai-${name}.md).toBe(true)` for 12; `:104-118` asserts `mode: all`; parity `:292-313` parses all frontmatter and asserts `model` matches `/^[a-z0-9-]+\/[a-z0-9.-]+$/` | ✅ PASS |
