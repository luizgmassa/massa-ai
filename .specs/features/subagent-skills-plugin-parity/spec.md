...
massa-th0th defines 12 reusable sub-agent skills in `skills/` (charter `SKILL.md` files) plus a single shipped subagent (`massa-th0th-navigator.md`) inside the Claude and Cursor plugins. None of the four supported plugin packages ship the 12 specialists in a host-native, invocation-ready form. Codex ships zero agents; Claude ships one; Cursor ships one; OpenCode ships none. Users installing any plugin therefore lose access to the investigator/planner/builder/reviewer/context-curator/verification-agent/requirements-analyst/architecture-specialist/test-engineer/documentation-agent/audit-specialist/mobile-specialist specialists that the massa-th0th workflow router and AGENTS.md registry describe.
...
- [ ] Preserve the existing `massa-th0th-navigator.md` subagent in Claude/Cursor (it is a different, index-first agent); the 12 new specialists coexist with it
...
| Feature | Reason |
| --- | --- |
| Rewriting massa-th0th workflows to invoke the shipped agents instead of inline prompt sections | Tracked separately by the registry's "Future Integration" note. This feature only ships the agent definitions. |
...
| Plugin-shipped agents using `hooks`/`mcpServers`/`permissionMode` frontmatter on Claude Code | Claude Code blocks these frontmatter fields on plugin-shipped agents for security. We omit them (agents inherit MCP from the parent session, which already loads the massa-th0th MCP server). |
| Changing the shared `massa-th0th-hook.ts` binary or `EVENT_MAP` | No hook work in this feature. |
...
| Removing or rewriting the existing `massa-th0th-navigator.md` | It is a distinct index-first navigator. Keep it; the 12 specialists are additive. |
...
| Full native frontmatter adaptation | Yes — map `metadata.permission` → host tool/permission lists; `model` PINNED per host (see model tables below, NOT advisory); strip massa-th0th-internal sections into advisory body prose | User selected "Full native adaptation". Preserves the read-only/write boundary central to the registry; drops fields hosts ignore. Model is pinned (user follow-up directive), not advisory. | y (user) |
...
| Codex agent file location vs plugin dir | Codex custom agents live in `<codex_dir>/agents/*.toml`, separate from `<codex_dir>/plugins/massa-th0th/`. The Codex plugin installer SHALL write agent TOML files alongside copying the plugin bundle. Uninstall removes only ownership-marked agent files. | Codex plugin discovery loads `skills/` + `hooks/` + `.mcp.json` from the plugin dir, but custom agents are loaded from the `agents/` directory at the config root, not the plugin dir. This is a divergence from Claude/Cursor (whose plugins bundle `agents/`). | y (web research) |
...
| Existing `massa-th0th-navigator.md` | Keep as-is in Claude/Cursor. Do NOT ship it to Codex/OpenCode (Codex has no plugin-bundled agents; OpenCode gets the 12 specialists but navigator is a Claude/Cursor-specific index-first agent already covered by the 6 slash commands). | Navigator is index-first and tied to MCP tools already registered. The 12 specialists are the gap this feature closes. | y (codebase) |
| Agent body content | Each agent body = charter Mission + Responsibilities + Restrictions + Inputs + Outputs + Invocation (use/do-not-use) + a condensed "massa-th0th Integration" section (Context Firewall, Verification Ladder, Memory Boundary, Synapse where relevant). Strip `license`/`metadata`/`Model Hint` frontmatter (move model hint into body as a comment). | Preserves the operational guidance; drops frontmatter hosts ignore or that would break loading. | y (user: full native adaptation) |
...
| Ownership marker for out-of-plugin-dir agents (Codex/OpenCode) | Codex TOML agents get a comment line `# massa-th0th-owned` at the top of the file; OpenCode `.md` agents get a frontmatter field `metadata: { massa-th0th-owned: true }` (hosts ignore unknown frontmatter). Uninstall removes only files carrying the marker. Claude/Cursor agents live inside the plugin dir and are removed with it (no per-file marker needed, but we still prefix names `massa-th0th-`). | Plan-critic F5: Codex/OpenCode agent dirs are SHARED with user agents; the `.massa-th0th.bak-<ts>` backup convention doesn't mark the agent file itself. Need an in-file marker for safe scoped uninstall. | y (plan-critic) |
| Claude `tools` frontmatter format | JSON array form `["Read", "Grep", "Glob", "Bash"]` (matching the existing `massa-th0th-navigator.md` which uses `["mcp__massa-th0th__*", "Read", "Grep", "Glob", "Bash(pwd)"]`). NOT the comma-string form. | Plan-critic F6: the existing navigator uses array form; mixing formats risks a parse path breaking. Consistency with the existing precedent. | y (codebase) |
...
**User Story**: As a Claude Code user with the massa-th0th plugin installed, I want the 12 sub-agent specialists available as native subagents (invocable via the Agent tool / @mention) with correct read-only/write tool boundaries, so that the massa-th0th workflow router's delegation model works inside Claude Code.

**Why P1**: Claude Code is the canonical host the plugins were built on first; it has the richest subagent support and the existing `massa-th0th-navigator.md` precedent.

**Acceptance Criteria**:

1. WHEN a user runs `apps/claude-plugin/install.sh --user` THEN the system SHALL create 12 agent files at `~/.claude/agents/massa-th0th-<name>.md` (names: investigator, planner, builder, reviewer, context-curator, verification-agent, requirements-analyst, architecture-specialist, test-engineer, documentation-agent, audit-specialist, mobile-specialist), each with `name`, `description`, `tools`, and `model` frontmatter
...
5. WHEN a user runs `apps/claude-plugin/install.sh --uninstall` THEN the system SHALL remove only the 12 ownership-marked `massa-th0th-*.md` agent files, preserving `massa-th0th-navigator.md` (not ownership-marked by this feature) and any user agents
...
7. WHEN the shipped agent files are compared to the generator output from the current `skills/*/SKILL.md` charters THEN every shipped `massa-th0th-*.md` body SHALL be byte-identical to the generator output (drift fails the test) — applies to all four hosts
...
**Independent Test**: Install the plugin into a temp HOME, list `~/.claude/agents/`, confirm exactly 12 `massa-th0th-*.md` files exist with the exact names, correct array-form `tools` frontmatter per the permission mapping, no `hooks`/`mcpServers`/`permissionMode` fields, and `model` matching the Claude pinning table. Run the generator and assert no diff.
...
1. WHEN a user runs `apps/codex-plugin/install.sh --user` THEN the system SHALL create 12 agent TOML files at `~/.codex/agents/massa-th0th-<name>.toml`, each with `name`, `description`, `developer_instructions`, and `sandbox_mode` fields
...
4. WHEN the Codex plugin is installed THEN each agent TOML's `developer_instructions` SHALL contain the charter body (Mission, Responsibilities, Restrictions, Inputs, Outputs, Invocation, massa-th0th Integration) as a triple-quoted TOML string
5. WHEN a user runs `apps/codex-plugin/install.sh --uninstall` THEN the system SHALL remove only the 12 ownership-marked `~/.codex/agents/massa-th0th-*.toml` files, preserving any user agent TOML files
6. WHEN a user re-runs the installer THEN it SHALL be idempotent (owned TOML files overwritten with identical content)
7. WHEN each Codex agent TOML is parsed THEN the `developer_instructions` string SHALL round-trip without TOML syntax errors (embedded `"""` escaped as `\"\"\"`) AND each TOML file SHALL carry a top comment `# massa-th0th-owned` for uninstall scoping
...
**Independent Test**: Install into a temp HOME, list `~/.codex/agents/`, confirm exactly 12 `massa-th0th-*.toml` files exist with `sandbox_mode` set per the permission mapping, a `# massa-th0th-owned` top comment, `model` + `model_reasoning_effort = "max"` per the Codex pinning table, and parse cleanly via a TOML parser. Run the generator and assert no diff.
...
**Why P3**: Cursor currently ships only `massa-th0th-navigator.md`. Cursor auto-discovers `agents/` in a registered plugin path, so the 12 new agents bundle into the existing plugin dir.

**Acceptance Criteria**:

1. WHEN a user runs `apps/cursor-plugin/install.sh --user` THEN the system SHALL create 12 agent files at `~/.cursor/plugins/massa-th0th/agents/massa-th0th-<name>.md`, each with `name`, `description`, `tools`, `model` frontmatter (same shape as Claude Code)
...
4. WHEN the Cursor plugin is installed THEN the existing `massa-th0th-navigator.md` SHALL remain in the plugin's `agents/` directory alongside the 12 new specialists
...
6. WHEN the shipped Cursor agent files are compared to the generator output from the current charters THEN every `massa-th0th-*.md` SHALL be byte-identical (drift fails the test)
...
**Independent Test**: Install into a temp HOME, list `~/.cursor/plugins/massa-th0th/agents/`, confirm 13 `.md` files (12 specialists + navigator) exist with correct array-form `tools` frontmatter and `model` matching the Cursor pinning table. Run the generator and assert no diff on the 12 specialists.
...
1. WHEN a user runs the OpenCode agent install command (new `agents` subcommand on `massa-th0th-config` OR a dedicated install script) with user scope THEN the system SHALL create 12 agent files at `~/.config/opencode/agents/massa-th0th-<name>.md`, each with `description` (required), `mode: subagent`, `model`, and `permission` frontmatter
...
4. WHEN the OpenCode agents are installed THEN each agent's markdown body SHALL contain the charter's Mission/Restrictions/Inputs/Outputs/Invocation/massa-th0th Integration content
5. WHEN a user runs the uninstall command THEN the system SHALL remove only the 12 ownership-marked `massa-th0th-*.md` files, preserving any user agents
6. WHEN a user re-runs the install THEN it SHALL be idempotent (owned files overwritten with identical content)
7. WHEN each OpenCode agent `.md` file is read THEN it SHALL carry `metadata: { massa-th0th-owned: true }` in frontmatter (hosts ignore unknown fields) for uninstall scoping AND the `bash` permission SHALL match the per-agent mapping (strict read-only → `bash: deny`; inspection-capable → `bash: { "*": "ask" }`; write → `bash: allow`)
8. WHEN the shipped OpenCode agent files are compared to the generator output from the current charters THEN every `massa-th0th-*.md` SHALL be byte-identical (drift fails the test)
...
**Independent Test**: Run the OpenCode agent install into a temp HOME, list `~/.config/opencode/agents/`, confirm exactly 12 `massa-th0th-*.md` files exist with `mode: subagent`, `metadata: { massa-th0th-owned: true }`, correct `permission` per the per-agent mapping, and `model` matching the OpenCode pinning table. Run the generator and assert no diff.

---

### P5: Installer menu + docs parity

**User Story**: As a user setting up massa-th0th, I want the root `install.sh` plugin menu and README to document the 12 subagent specialists shipped per tool, so that I know they exist and how to invoke them. I also want `FEATURES.md` to document the full depth (the 12 names, per-host model + effort pinning, permission boundaries, file locations, ownership markers, and the generator/parity-test contract), with the root `README.md` carrying a concise summary that links into `FEATURES.md` for depth.
...
- WHEN a user has an existing `massa-th0th-<name>.md`/`.toml` without the ownership marker THEN the installer SHALL back it up (`.massa-th0th.bak-<ts>`) before overwriting
...
- WHEN a user runs `--uninstall` THEN only ownership-marked `massa-th0th-*` agent files SHALL be removed; the unmarked `massa-th0th-navigator.md` (Claude/Cursor) and any user agents SHALL survive
- WHEN Codex `agents/` directory has user TOML files THEN the installer SHALL only write/overwrite the 12 `massa-th0th-*` files, preserving user agents
- WHEN OpenCode `agents/` directory has user `.md` files THEN the installer SHALL only write/overwrite the 12 `massa-th0th-*` files
...
- [ ] Install is idempotent; uninstall removes only ownership-marked agent files, preserving user agents and the existing `massa-th0th-navigator.md`
- [ ] Existing `massa-th0th-navigator.md` (Claude/Cursor) is preserved; the 12 specialists are additive
