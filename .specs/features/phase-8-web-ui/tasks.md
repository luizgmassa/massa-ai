...
**Files:**
- `apps/web-ui/package.json` — `@th0th-ai/web-ui`, private, zero deps, script
...
- `apps/web-ui/src/static/app.js` — exports (for testing) + wires the app:
  - `api(path, { method, body })` — fetch wrapper; reads `TH0TH_API_BASE` from
    `window.__TH0TH_API_BASE__` or `""` (same-origin); injectable for tests.
