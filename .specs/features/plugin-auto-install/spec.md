...
13. WHEN the plugin phase is reached through the harness — the root `install.sh` `k)` menu (which runs `install-harness.sh`), `scripts/setup-local-first.sh`, or a direct `install-harness.sh --plugins` invocation — THEN the identical detection + version gating SHALL apply (single shared implementation). The root `p)` menu deliberately invokes the per-host installers directly as the manual install surface (PAI-08, goal 3); it does not route through the harness and is un-gated by design. (Reworded 2026-07-29 at independent validation: the original "`k)`/`p)` menus … reach the plugin phase" conflicted with PAI-08's menu-string freeze and goal 3; the implementation follows PAI-08/goal 3.)
