...
### T4 Implementation Notes (as built)

- **Trigger coverage boundary.** The guard trigger resolves aliases on INSERT and on UPDATE of the identity column under a shared advisory lock. Under READ COMMITTED (the app default; no code sets a higher level) the VOLATILE `project_identity_resolve` runs with a fresh per-command snapshot AFTER the lock wait, so a writer that blocked on apply's exclusive lock sees the just-committed alias. What the trigger cannot rewrite: statements FILTERED by a retired id (`UPDATE/DELETE ... WHERE project_id='retired'` matches 0 rows) and payload-embedded ids (`scheduled_jobs.payload` has no identity column).
- **Application adapter resolution (user decision 2026-07-20: implement now).** `ProjectIdentityAliasResolver` (TTL cache, 250 ms bounded lookup, FAIL-OPEN with warn, post-commit invalidation of both ids via the registry) is called at every write-entry seam: ETL `runInternal`, `indexFileImpl`, `IndexManager.updateIndexMetadata`, observation/checkpoint/synapse-session/scheduler-job persist closures, `PgJobStore.persist`, `PgHandoffStore.insert`, `PgProposalStore.insert`, `MemoryRepositoryPg.insert`/`deleteByProject`. Sync-mirror stores resolve inside the async persist closure: the mirror keeps the caller's id by design, the durable row lands on the target.
- **Guard installation.** Startup installs on all mutable direct stores + `operation_log` (so NEW audit rows resolve; history is never rewritten) + existing `vector_documents*`; runtime create sites (`keyword_documents`, `search_cache`, `search_analytics`, `search_events`, `vector_documents_<dim>d`) install their own guard during initialization. `project_identity_operations` is unguarded (no `project_id` column).
- **Queue drain adjudication.** Explicit drain is replaced by trigger resolution for INSERT-class queued writes plus seam resolution for filtered/payload writes; in-memory queue payloads never persist a retired identity.
