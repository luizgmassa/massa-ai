...
| AC8 | CRS-08 | PASS (amended 2026-08-05) | `apps/cursor-plugin/__tests__/manifest.test.ts:121-139` — directory layout test asserts `skills/`, `skills/*/SKILL.md` (6), `hooks/hooks.json`, `mcp.json`, `agents/massa-ai-navigator.md` all exist. **Amendment (debug-harness-install-cursor-opencode):** the original PASS rested on `vscode.cursor.plugins.registerPath` auto-discovery, which was never verified against live Cursor and is disproven — Cursor has no plugin discovery; subagents load only from flat `.cursor/agents/*.md`. The bundle-layout assertion stands as a packaging check; the discovery claim it cited does not. Install now copies agents to `~/.cursor/agents/` (`install.test.ts`, CRS-01/04 tests). |
