...
Dedicated DB: `postgresql://test:test@127.0.0.1:5433/massa_th0th_test`.
...
| G03 | Shared unit | `bun run --filter @massa-th0th/shared test` | explicit PG env | No | Yes | PASS — 11/0/0 |
| G04 | Core unit | `bun run --filter @massa-th0th/core test:unit` | explicit PG; E2E off; LLM off | No | Yes | PASS — 96 files, 25 isolation groups; 2 expected code-model skips |
| G05 | MCP unit | `bun run --filter @massa-th0th/mcp-client test` | neutral | No | Yes | PASS — 7/0/0 |
| G06 | Tools API unit | `bun run --filter @massa-th0th/tools-api test` | explicit PG | No | Yes | PASS — 52/0/0 after host-network rerun |
| G07 | OpenCode unit | `bun run --filter @massa-th0th/opencode-plugin test` | explicit PG | No | Yes | PASS — 18/0/0 |
| G08 | Script tests | `bun run test:scripts` | host shell | No | No | PASS — 8/0/0 |
| G09 | Core integration | `bun run --filter @massa-th0th/core test:integration` | API `:3334`, PG `:5433`, Ollama | No | Maybe | PASS — 23 expected RUN_E2E-off skips |
| G10 | Standard E2E | `bun run --filter @massa-th0th/core test:e2e` | API `:3334`, PG `:5433`, MCP, Ollama `:11435` | Prefix-scoped | No | DOCUMENTED EXCEPTION — full default-qwen run passed before parity amendments; post-amendment bge run completed all groups but two qwen-calibrated quality gates failed; final default-qwen cold run hit the 420s indexing deadline before assertions (E-08/E-09). Memory matrix fix passed live 25/25; all changed PG subsystems have focused evidence |
