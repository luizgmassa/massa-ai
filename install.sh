...
# Ask every Config-tab feature toggle, prefilled from the config.json already
# on disk. Replaces the old two-question search-quality prompt: nine of the
# Admin Portal's sections were decided here and never shown, and this ran only
# on a first install, so there was no supported way to change an answer later.
#
# The questions themselves live in scripts/lib/installer-feature-prompts.sh so
# install.sh and setup-local-first.sh ask the same ones — the two prompt blocks
# were previously pasted copies that had already drifted in their tty handling.
prompt_feature_flags() {
  local dir="$1"
  local llm_on="$2"

  # Same load-or-fetch shape as load_installer_env_transaction: docker mode has
  # no scripts/ tree until it fetches one, and `curl | bash` has no reliable
  # $BASH_SOURCE to resolve a sibling from.
  if ! declare -F installer_feature_flow >/dev/null 2>&1; then
    local shared="${dir}/scripts/lib/installer-shared.sh"
    local prompts="${dir}/scripts/lib/installer-feature-prompts.sh"
    local fetched=""

    # shellcheck source=scripts/lib/installer-shared.sh
    [ -r "$shared" ] && source "$shared"

    if [ -r "$prompts" ]; then
      # shellcheck source=scripts/lib/installer-feature-prompts.sh
      source "$prompts"
    else
      fetched="$(mktemp "${dir}/.installer-feature-prompts.XXXXXX")"
      if curl -fsSL "${GITHUB_RAW}/${BRANCH}/scripts/lib/installer-feature-prompts.sh" \
        -o "$fetched" 2>/dev/null; then
  ...
  # A prompt library that could not be loaded must not fail the install — it
  # only means there is nothing to ask, so the template's own defaults stand.
  if ! declare -F installer_feature_flow >/dev/null 2>&1; then
    warn "Feature prompts unavailable — installing with default feature flags."
    SEARCH_QU_ENABLED=${SEARCH_QU_ENABLED:-false}
    SEARCH_RERANK_ENABLED=${SEARCH_RERANK_ENABLED:-false}
    return 0
  fi

  installer_feature_flow "${HOME}/.config/massa-ai/config.json" "$llm_on"
}
...
write_env() {
  ...
  prompt_feature_flags "$dir" "$llm_enabled"
  local qu_enabled="${SEARCH_QU_ENABLED:-false}"
  local rerank_enabled="${SEARCH_RERANK_ENABLED:-false}"
  # The answers reach .env through the variables the writer below interpolates.
  # `llm_enabled` stays the detection result; the prompt can only narrow it,
  # never enable a model that is not pulled.
  [ "${LLM_ENABLED:-true}" = "false" ] && llm_enabled=false
...
HOOKS_BRIDGE_ENABLED=${HOOKS_BRIDGE_ENABLED:-true}
HOOKS_BRIDGE_MIN_OBS=8
HOOKS_BRIDGE_MIN_INTERVAL_MS=300000
HOOKS_BRIDGE_MAX_WINDOW=8

# ── Cross-session handoffs ────────────────────────────────────
HANDOFFS_ENABLED=${HANDOFFS_ENABLED:-true}

# ── Project bootstrap ─────────────────────────────────────────
BOOTSTRAP_ENABLED=${MEMORY_BOOTSTRAP_ENABLED:-true}
...
AUTO_IMPROVE_ENABLED=${MEMORY_AUTO_IMPROVE_ENABLED:-true}
...
install_plugins_menu() {
  ...
  while true; do
    echo ""
    echo -e "${BOLD}Install massa-ai plugins (skills + hooks + MCP + 17 subagent specialists):${NC}"
    echo -e "  ${CYAN}1)${NC} Claude Code plugin (skills + commands + 17 subagent specialists + hooks auto-write)"
    echo -e "  ${CYAN}2)${NC} Codex plugin (6 skills, 6 hook events, MCP, 17 subagent specialists)"
    echo -e "  ${CYAN}3)${NC} Cursor plugin (6 skills, 7 hook events, MCP, 17 subagent specialists)"
    echo -e "  ${CYAN}4)${NC} OpenCode plugin (npm install + 17 subagent specialists via config CLI)"
...
print_opencode_plugin_instructions() {
  ...
  echo ""
  echo -e "  ${CYAN}3. Install the 17 subagent specialists:${NC}"
