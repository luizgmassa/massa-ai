 ...
 *
 * The twin of `apps/mcp-client/src/__tests__/config-cli-doctor.test.ts`:
 * `runtimeDriftReport` is mocked (same pattern as config-cli-profile.test.ts
 * in this package) — the real doctor reads the operator's real home, and a
 * test must never touch `~/.claude`, `~/.config/massa-ai`, or the installed
 * variant trees. The mocked report drives the three output branches
 * (healthy, drift, override) and the `--fix` flow (switch invoked with the
 * RECORDED profile, re-report afterwards, exit codes). The --fix path's
 * mutation is `switchProfile`, which is mocked here — its own behavior is
...
describe("massa-ai-config doctor", () => {
  ...
  test("version drift is reported with the update-the-plugin remedy", async () => {
    runtimeDriftReport.mockImplementation(() =>
      report({ versionDrift: true, sourceVersion: "1.61.0", stateVersion: "1.60.1" }),
    );
    const { code, out } = await captureConsole(() => runCli(["doctor"]));
    expect(code).toBe(0);
    expect(out).toContain("drift: live tree 1.61.0 != recorded 1.60.1");
    expect(out).toContain("update the plugin");
  });

  test("a thrown report surfaces as Error output and exit 1", async () => {
    runtimeDriftReport.mockImplementation(() => {
      throw new Error("boom");
    });
    const { code, err } = await captureConsole(() => runCli(["doctor"]));
    expect(code).toBe(1);
