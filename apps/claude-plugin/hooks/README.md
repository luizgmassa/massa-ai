# th0th Hook Scripts (Passive Capture)

These POSIX shell scripts wire Claude Code lifecycle hooks into the th0th
...
| Script | Claude Code hook | th0th event |
| --- | --- | --- |
...
- **Config via env:** `TH0TH_API_BASE` (default `http://localhost:3333`),
  `TH0TH_API_KEY` (optional), `TH0TH_PROJECT_ID` (optional; defaults to the
...
4. Run a Claude Code session — Observation rows appear in
   `~/.rlm/observations.db` and are consolidated into memories when the LLM is
