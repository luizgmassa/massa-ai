...
if [ -n "$_massa_th0th_raw" ] && command -v jq >/dev/null 2>&1; then
  _massa_th0th_session=$(printf '%s' "$_massa_th0th_raw" | jq -r '.session_id // .sessionId // ""' 2>/dev/null)
elif [ -n "$_massa_th0th_raw" ]; then
  _massa_th0th_session=$(printf '%s' "$_massa_th0th_raw" | sed -n 's/.*"session_id"[: ]*"*\([^",}]*\).*/\1/p' 2>/dev/null)
fi
...
_massa_th0th_session_field=""
if [ -n "$_massa_th0th_session" ]; then
  _massa_th0th_session_field=$(printf ',"sessionId":"%s"' "$_massa_th0th_session")
fi
...
_massa_th0th_body=$(printf '{"event":"%s","projectId":"%s"%s,"payload":%s}' \
  "$EVENT" "$_massa_th0th_project" "$_massa_th0th_session_field" "$_massa_th0th_stdin")
