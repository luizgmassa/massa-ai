 ...
 * drift (live bundle vs recorded state), agent drift (active reviewer
...
export function buildSessionStartDoctorLine(
  ...
  if (typeof profile === "string" && profile) {
    const activeModel = readModelLine(path.join(pluginRoot, "agents", "reviewer.md"));
    const variantModel = readModelLine(
      path.join(pluginRoot, "agent-profiles", profile, "reviewer.md"),
    );
    if (activeModel && variantModel && activeModel !== variantModel) {
      lines.push(
        `[massa-ai] agent drift: reviewer ${activeModel} vs ${profile} variant ${variantModel} — re-run the profile switch`,
      );
    }
  }
