#!/bin/sh
# Shared helper: per-session project id pinning for massa-th0th hook scripts.
#
...
#
#   massa_th0th_pin_project_id <session_id> <cwd>
#
# echoes the pinned project id. First event of a session computes the id
# (MASSA_TH0TH_PROJECT_ID env override > git toplevel basename > cwd basename)
...
#
# Pin files live under ${TMPDIR:-/tmp}/massa-th0th-hooks/<sanitized-session_id>.

massa_th0th_pin_project_id() {
  _mpin_session="$1"
  _mpin_cwd="$2"
  ...
  if [ -n "$_mpin_session" ]; then
    _mpin_dir="${TMPDIR:-/tmp}/massa-th0th-hooks"
  ...
  _mpin_computed="${MASSA_TH0TH_PROJECT_ID:-}"
