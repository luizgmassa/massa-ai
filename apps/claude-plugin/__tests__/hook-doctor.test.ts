...
describe("buildSessionStartDoctorLine", () => {
  test("healthy install (versions agree, no agent drift, no override) → null", () => {
    ...
    const agent = "---\nname: reviewer\nmodel: glm-5.3-flash\neffort: max\n---\nbody";
    writeText(path.join(pluginRoot, "agents", "reviewer.md"), agent);
    writeText(path.join(pluginRoot, "agent-profiles", "work", "reviewer.md"), agent);
  ...
  test("active reviewer model ≠ recorded profile variant → agent drift line", () => {
    writeJson(path.join(pluginRoot, ".claude-plugin", "plugin.json"), { version: "1.57.0" });
    writeJson(getInstallStatePath(), {
      version: 2,
      platforms: { claude: { plugin: { version: "1.57.0" }, modelProfile: { profile: "work" } } },
    });
    writeText(path.join(pluginRoot, "agents", "reviewer.md"), "---\nmodel: glm-5.3-flash\n---\n");
    writeText(path.join(pluginRoot, "agent-profiles", "work", "reviewer.md"), "---\nmodel: glm-5.2\n---\n");
...
describe("drift sentinel names a shipped agent (NAM AC-8)", () => {
  test("the agent file the hook reads exists in the generated Claude bundle and every variant", () => {
    ...
    expect(sentinels).toEqual(["reviewer.md"]);
