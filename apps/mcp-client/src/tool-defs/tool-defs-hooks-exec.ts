 ...
 *        handoff_update, handoff_delete, list_proposals, approve_proposal,
 *        reject_proposal, create_proposal, update_proposal, delete_proposal,
 *        execute, execute_file, batch_execute, fetch_and_index
 *
 * handoff_update/handoff_delete and create_proposal/update_proposal/
 * delete_proposal (T10, HPC-05/AC-05.1) are the MCP-parity counterparts of
 * the Phase 3 portal CRUD routes (`routes/handoff.ts` PATCH/DELETE `/:id`,
 * `routes/proposals.ts` POST `/create` + PATCH/DELETE `/:id`). The `:id`
 * PATCH/DELETE shape follows `synapse_update`/`synapse_end`
 * (tool-defs-synapse.ts) — the existing proof that pattern works end to end
 * through `call-tool-proxy.ts`, which needs no change for it.
 */
...
export const HOOKS_EXEC_TOOL_DEFINITIONS: ToolDefinition[] = [
  ...
  },
  {
    name: "handoff_delete",
    description:
      "Hard-delete a handoff by id, permitted in any status including open. The dual-written memory row is left intact (dangling metadata.handoffId is accepted, documented behaviour). Missing id or project mismatch return {ok:false, reason}. Never throws.",
    apiEndpoint: "/api/v1/handoff/:id",
    apiMethod: "DELETE",
    inputSchema: {
      type: "object",
  ...
  },
  {
    ...
    description:
      "Create a pending auto-improvement proposal by hand. kind must be memory.create/memory.update/memory.tag; payload is validated against the same per-kind rules the store enforces on read, so a write the reader would reject is refused here instead. Never throws.",
  ...
  },
  {
    ...
    description:
      "Hard-delete a proposal by id, permitted in any status. Deleting an already-approved proposal does not reverse the memory edit it already applied — the response says so explicitly via a note field. Missing id or project mismatch return {ok:false, reason}. Never throws.",
    apiEndpoint: "/api/v1/proposal/:id",
    apiMethod: "DELETE",
    inputSchema: {
      type: "object",
