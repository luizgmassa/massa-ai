...
if curl -s --connect-timeout 3 "http://${WINDOWS_IP}:11434/api/tags" > /dev/null 2>&1; then
    ...
    if [[ "$choice" == "y" || "$choice" == "Y" ]]; then
        ...
        echo ""
        echo "Pulling embedding model qwen3-embedding:4b..."
...
if curl -s "${OLLAMA_URL}/api/embeddings" \
    -d '{"model": "qwen3-embedding:4b", "prompt": "test"}' > /dev/null 2>&1; then
    echo "Embeddings working."
else
    echo "Could not reach embedding endpoint. Make sure qwen3-embedding:4b is pulled:"
