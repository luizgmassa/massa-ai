...
export function renderModelRegistry(data: RegistryPayload | null | undefined, opts?: RegistryRenderOpts | null): string {
  ...
  const helpSection = '<details class="help-card"><summary>About this tab</summary>' +
    ...
    '<p>Picks a different model for one agent on one tool, within the selected profile — the only way to run, for example, <code>builder</code> on a stronger model than the rest of the profile. Pick <code>Profile default</code> to remove the override and inherit the profile\'s own model for that tool. Read-only agents carry no override in the built-in profiles, so they always resolve to the profile default — by convention the default should be the profile\'s strongest model.</p>' +
    '<h4>Save &amp; Apply</h4>' +
