...
| **MCP** | stdio server exposing 59 tools |
...
| **MCP Stdio Transport** (`apps/mcp-client/src/`) | 15 | MCP stdio server, 59 tools, tool-def JSON Schemas, and the dual-client contract. |
...
| **Agent-harness surface** | `skills/AGENTS.md`, `CLAUDE.md` | 7 sub-agent specialists, 4 plugin hosts, generated artifacts with parity tests. `apps/*-plugin/skills/` are byte-identical generated copies — never edit them directly, edit `skills/` and regenerate. |
