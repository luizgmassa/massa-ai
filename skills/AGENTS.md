...
When modifying `CHANGELOG.md`, always:

- Write entries under `## [Unreleased]`, never into an already-released section.
- Choose the heading that matches your change:
  - `### Added`, `### Changed`, `### Removed`, `### Deprecated` → **minor** version bump
  - `### Fixed`, `### Security` → **patch** version bump
  - **The heading you choose decides the released version.** A non-empty Added/Changed/Removed/Deprecated section triggers a minor bump (e.g., 1.2.0 → 1.3.0). Only Fixed/Security triggers a patch bump (e.g., 1.2.0 → 1.2.1). Empty `[Unreleased]` produces no release.
- **Never commit empty headings.** A `### Added` with no bullets under it is ignored by the parser, so a stray empty heading cannot force a release. If you add a section but don't fill it yet, remove the heading or leave it for later.
- **Released sections are written by automation.** `## [X.Y.Z] - DATE` entries are generated by `scripts/release-version.ts` and committed by `.github/workflows/release.yml`. Never hand-edit a released section. Never hand-edit the version in `package.json` — let automation handle it.
- **The CI merge gate enforces CHANGELOG maintenance.** A PR that does not modify `CHANGELOG.md` fails unless it carries the `no-changelog` label (bots exempt). `no-changelog` is for docs-only or chore-only changes that should not cut a release.
- **Major bumps are manual.** A `2.0.0` requires hand-editing `package.json`. The auto-release process never increments the major component.

Full authoring rules live in `CLAUDE.md` under "### Releasing and CHANGELOG authoring".
