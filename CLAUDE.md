...
`skills/AGENTS.md` is the registry for the 16 sub-agent specialists **and** the single
...
`synapse-usage` skills were folded into those two references and deleted. `apps/{claude,codex,cursor,opencode}-plugin/` ship those same 16
...
The installers are **bash**, not TypeScript: `scripts/install-skills.sh` (symlinks +
`AGENTS.md` bootstrap; `--apply/--uninstall/--dry-run/--check`),
...
Entries always go in `## [Unreleased]` under one of the standard section headings:

- `### Added` — new features (→ **minor** bump)
- `### Changed` — enhancements to existing features (→ **minor** bump)
- `### Removed` — breaking changes (→ **minor** bump)
- `### Deprecated` — marked-for-deletion features (→ **minor** bump)
- `### Fixed` — bug fixes (→ **patch** bump)
- `### Security` — security fixes (→ **patch** bump)

**The heading you choose decides the released version.** A non-empty `### Added`,
`### Changed`, `### Removed`, or `### Deprecated` is a **minor** bump (e.g., `1.2.0` →
`1.3.0`). Only `### Fixed` and/or `### Security` is a **patch** bump (e.g., `1.2.0` →
`1.2.1`). If both a minor-class and patch-class heading have content, **minor wins**.
An empty `[Unreleased]` (no heading with at least one bullet) produces **no release**
(the run ends green with no tag).

**Headings with no bullets are ignored.** If you add a section heading but don't fill it
yet, leave it out of `[Unreleased]` to avoid accidentally forcing a release. Never
commit an empty `### Added` or other heading.

**Released sections are written by automation.** The entry `## [X.Y.Z] - DATE` and the
content beneath it are generated by `scripts/release-version.ts` and committed by
`release.yml`. Never hand-edit a released section; future releases will overwrite it.
Never hand-edit the version in `package.json` — `release-version.ts` does it. For a
routine change, write under `## [Unreleased]` and let the release process handle the
rest.

**The CI merge gate enforces CHANGELOG maintenance.** A PR that does not modify
`CHANGELOG.md` fails unless it carries the `no-changelog` label (bots exempt).
`no-changelog` is the path for docs-only, chore-only, or automated-test-only changes
that should not cut a release. With that label, an empty `[Unreleased]` is allowed.

**Major bumps are manual.** The auto-release process never increments the major
component. A `2.0.0` requires a manual edit to `package.json`.
