...
**[`CONTRIBUTING.md`](./CONTRIBUTING.md) owns the CHANGELOG authoring rules** — which
heading to file an entry under, that the heading decides the released version, empty-
heading handling, and what `no-changelog` means. Read it before writing an entry; do not
restate its rules here. One-line summary: the heading you pick *is* the version bump.
