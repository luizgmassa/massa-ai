...
`skills/AGENTS.md` is the registry for the 7 sub-agent specialists **and** the single
source of the Conversation Feedback policy (the
...
`synapse-usage` skills were folded into those two references and deleted. `apps/{claude,codex,cursor,opencode}-plugin/` ship those same 7
...
There is no allowlist exception left: the former index-first agent's allowlist
(`AGENT_TOOLS_OVERRIDE`) retired with the roster consolidation, and `code-explorer` keeps
index-first retrieval as charter prose under the ordinary denylist. The other three hosts
...
(rank 4, the terminal literal fallback since v2 dropped the registry's `hostDefaults` key).
An unknown name at the flag or env rank still throws before any file is written; a stale
recorded rank-3 profile (removed, renamed, or no longer supporting this host) instead
degrades and falls through to rank 4, since it reflects a historical switch this run did not
request (agent-drift followup T1's `validStateProfile`, fix round 2026-09-23). `validateRegistry`
still reports every violation in one throw. Adding a `MASSA_AI_*` knob here
...
`massa-ai-owned: true` marker is a body comment and not `metadata:` — every `.md` host
carries it as the first body line, and installers, `config-cli.ts` `agents install|uninstall`,
and the profile-switch engine select owned files through the content-marker predicate
`isOwnedAgentFile` in `packages/shared/src/profile-switch/ownership.ts` (the bash installers
inline the same predicate), so deleting it orphans installed agents; Cursor publishes no model-ID table
