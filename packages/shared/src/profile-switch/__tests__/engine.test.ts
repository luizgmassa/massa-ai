...
describe("listProfiles", () => {
  test("lists available profiles per host purely from on-disk variant dirs, no registry access", () => {
    ...
    stageVariant(home, "claude", "work", { "judge.md": "w" });
    stageVariant(home, "codex", "balanced", { "judge.toml": "b" });
    stageActive(home, "claude", { "judge.md": "b" });
  ...
  test("reports recorded active profile and bundle version from existing state", () => {
    stageVariant(home, "claude", "home", { "judge.md": "h" });
    writeState(home, {
      version: 2,
      platforms: {
        claude: {
...
describe("switchProfile — happy path", () => {
  test("switches a file-route host, overwrites only massa-ai-owned files, records state only after copies", () => {
    stageVariant(home, "claude", "work", { "judge.md": "opus" });
    stageActive(home, "claude", { "judge.md": "sonnet" });
    ...
    expect(readBody(path.join(activeDir, "judge.md"))).toBe("opus");
  ...
  test("host filter switches only the requested host", () => {
    stageVariant(home, "claude", "work", { "judge.md": "opus" });
    stageActive(home, "claude", { "judge.md": "sonnet" });
    stageVariant(home, "codex", "work", { "judge.toml": "opus" });
    stageActive(home, "codex", { "judge.toml": "sonnet" });
    writeState(home, {
      version: 2,
      platforms: {
    ...
    expect(readBody(path.join(codexActive, "judge.toml"))).toBe("sonnet"); // untouched
  });
  ...
  test("dry-run changes nothing on disk", () => {
    ...
    const activeFile = path.join(layoutFor(home, "claude").activeDir, "judge.md");
...
describe("agent-runtime-drift (INV3 + claude drift row)", () => {
  ...
  test("claude row carries liveRoot/sourceVersion (registry-cache fixture) and envOverride; other rows null", () => {
    ...
    fs.writeFileSync(
      path.join(cacheRoot, "agents", "code-explorer.md"),
      `---\nmodel: glm-5.2\neffort: max\n---\n${OWNED_MARKER_MD}\nbody`,
    );
    writeState(home, {
      version: 2,
...
describe("switchProfile — Error Handling Strategy: unsupported host (MPS-09)", () => {
  test("a profile absent for one host reports 'unsupported' there while another host still switches", () => {
    stageVariant(home, "claude", "open_models", { "judge.md": "oss" });
    stageActive(home, "claude", { "judge.md": "opus" });
    stageVariant(home, "codex", "balanced", { "judge.toml": "b" }); // no open_models for codex
    stageActive(home, "codex", { "judge.toml": "b" });
    writeState(home, {
      version: 2,
      platforms: {
        claude: { root: "/x", skills: [], skillsOwner: "plugin", installRoute: "file" },
  ...
  test("a host whose bundle predates variants (no agent-profiles dir at all) reports the upgrade-plugin reason", () => {
    stageActive(home, "codex", { "judge.toml": "b" }); // active dir exists, no variants dir ever created
    writeState(home, {
      version: 2,
    ...
    stageVariant(home, "claude", "work", { "judge.md": "opus" });
    stageActive(home, "claude", { "judge.md": "s" });
    writeState(home, {
      version: 2,
      platforms: {
        claude: { root: "/x", skills: [], skillsOwner: "plugin", installRoute: "file" },
...
describe("switchProfile — Error Handling Strategy: marketplace-route refusal (F1)", () => {
  ...
  test('installRoute "marketplace" with no resolvable install root fails, naming the unresolved registry path', () => {
    ...
    // staged ~/.claude/agents/judge.md is untouched.
    const activeFile = path.join(layoutFor(home, "claude").activeDir, "judge.md");
...
describe("Claude marketplace parity (T18, CPP-03..06)", () => {
  test("(CPP-03) listProfiles reports installed:true + variants from the resolved marketplace root", () => {
    ...
    fs.writeFileSync(path.join(cacheRoot, "agent-profiles", "balanced", "judge.md"), owned("judge.md", "b"));
    fs.mkdirSync(path.join(cacheRoot, "agents"), { recursive: true });
    stageMarketplaceRegistry(home, cacheRoot);
    writeState(home, {
      version: 2,
  ...
  test("(CPP-04) switchProfile on a resolved marketplace root copies into <root>/agents and records modelProfile only after the copy", () => {
    ...
    fs.writeFileSync(path.join(cacheRoot, "agent-profiles", "work", "judge.md"), owned("judge.md", "opus"));
    fs.mkdirSync(path.join(cacheRoot, "agents"), { recursive: true });
    fs.writeFileSync(path.join(cacheRoot, "agents", "judge.md"), owned("judge.md", "sonnet"));
    stageMarketplaceRegistry(home, cacheRoot);
    writeState(home, {
      version: 2,
      platforms: { claude: { root: "/x", skills: [], skillsOwner: "plugin", installRoute: "marketplace" } },
    ...
    expect(readBody(path.join(cacheRoot, "agents", "judge.md"))).toBe("opus");
  ...
  test("(CPP-05) re-switching to the already-active marketplace profile still reports switched with a file count, never an error", () => {
    ...
    fs.writeFileSync(path.join(cacheRoot, "agent-profiles", "cheap", "judge.md"), owned("judge.md", "haiku"));
    fs.mkdirSync(path.join(cacheRoot, "agents"), { recursive: true });
    stageMarketplaceRegistry(home, cacheRoot);
    writeState(home, {
      version: 2,
  ...
  test("codex marketplace-route switch now proceeds (T2/MDS-02: the stale refusal is retired) and switches codex's file-route layout", () => {
    ...
    expect(report.hosts[0].filesChanged).toBe(1);
    const codexActive = layoutFor(home, "codex").activeDir;
    expect(readBody(path.join(codexActive, "judge.toml"))).toBe("opus");
  });
});
...
describe("switchProfile — T2b: runtime tracked-path guard (AC-02.4)", () => {
  ...
  test("a deliberately git add-ed destination file is refused, naming the offending path", () => {
    ...
    gitInit(home);
    const activeDir = layoutFor(home, "claude").activeDir;
    const destFile = path.join(activeDir, "judge.md");
...
describe("switchProfile — F3: OpenCode symlink repoint", () => {
  test("the active OpenCode agent stays a symlink post-switch, repointed at the new profile's variant file", () => {
    stageVariant(home, "opencode", "balanced", { "judge.md": "b" });
    stageVariant(home, "opencode", "work", { "judge.md": "w" });

    const layout = layoutFor(home, "opencode");
    fs.mkdirSync(layout.activeDir, { recursive: true });
    fs.symlinkSync(layout.variantDir("balanced") + "/judge.md", path.join(layout.activeDir, "judge.md"));
    ...
    const dest = path.join(layout.activeDir, "judge.md");
    expect(fs.lstatSync(dest).isSymbolicLink()).toBe(true);
    expect(readBody(dest)).toBe("w");
    expect(path.basename(fs.readlinkSync(dest))).toBe("judge.md");
  ...
  test("F3: a regular (non-symlink) file at the active path is never clobbered, mirroring install.sh's pre-flight", () => {
    ...
    const layout = layoutFor(home, "opencode");
    fs.mkdirSync(layout.activeDir, { recursive: true });
    fs.writeFileSync(path.join(layout.activeDir, "judge.md"), owned("judge.md", "user hand-edited this"));
...
describe("switchProfile — marker ownership (NAM AC-6, AC-11)", () => {
  ...
  test("claude: an unmarked same-named file in the active dir is left byte-identical", () => {
    ...
    const foreign = "---\nname: judge\n---\nmy own judge\n";
    fs.writeFileSync(path.join(activeDir, "judge.md"), foreign);
    fileRouteState("claude");

    const report = switchProfile({ profile: "work", host: "claude", targetHome: home });
    expect(report.hosts).toEqual([{ host: "claude", status: "switched", filesChanged: 0 }]);
    expect(fs.readFileSync(path.join(activeDir, "judge.md"), "utf-8")).toBe(foreign);
  });
  ...
  test("codex: an unmarked same-named TOML in the active dir is left byte-identical", () => {
    ...
    fs.mkdirSync(activeDir, { recursive: true });
    fs.writeFileSync(path.join(activeDir, "judge.toml"), 'name = "judge"\n');
    fileRouteState("codex");

    const report = switchProfile({ profile: "work", host: "codex", targetHome: home });
    expect(report.hosts).toEqual([{ host: "codex", status: "switched", filesChanged: 0 }]);
    expect(fs.readFileSync(path.join(activeDir, "judge.toml"), "utf-8")).toBe('name = "judge"\n');
  });

  test("claude: legacy-named and unmarked variant entries are never copied", () => {
    stageVariant(home, "claude", "work", { "judge.md": "opus", "massa-ai-planner.md": "stale legacy" });
    ...
    const report = switchProfile({ profile: "work", host: "claude", targetHome: home });
    expect(report.hosts).toEqual([{ host: "claude", status: "switched", filesChanged: 1 }]);
    expect(fs.readdirSync(layoutFor(home, "claude").activeDir)).toEqual(["judge.md"]);
  });

  test("opencode: a user symlink to an unmarked file elsewhere keeps its target; legacy variant entries are not linked", () => {
    stageVariant(home, "opencode", "work", { "judge.md": "w", "massa-ai-planner.md": "stale legacy" });
    const layout = layoutFor(home, "opencode");
    fs.mkdirSync(layout.activeDir, { recursive: true });
    const dotfile = path.join(home, "dotfiles", "opencode", "agents", "judge.md");
    fs.mkdirSync(path.dirname(dotfile), { recursive: true });
    fs.writeFileSync(dotfile, "---\ndescription: mine\n---\nmy judge\n");
    fs.symlinkSync(dotfile, path.join(layout.activeDir, "judge.md"));
    fileRouteState("opencode");

    const report = switchProfile({ profile: "work", host: "opencode", targetHome: home });
    expect(report.hosts).toEqual([{ host: "opencode", status: "switched", filesChanged: 0 }]);
    expect(fs.readlinkSync(path.join(layout.activeDir, "judge.md"))).toBe(dotfile);
    expect(fs.readdirSync(layout.activeDir)).toEqual(["judge.md"]);
  });
});
